...
function generateGrid(gridData,moduleURL,width,height){
	return $('#mytable').jqGrid(gridData)
		...
		.navButtonAdd('#pager',{
		   ...
		   onClickButton: function(){ 
			 popupForm(moduleURL + '/add/?width='+750+'&height='+525);
		   }, 
		   position:"last"
		})
		.navButtonAdd('#pager',{
		   ...
		   onClickButton: function(){ 
			...
			// alert(id);
			 if(id){
				 popupForm("<?=site_url($module_url)?>" + '/update/' + id + '/?width='+750+'&height='+525);
			 }else{
		...
		.navButtonAdd('#pager',{
		   ...
		   onClickButton: function(){ 
			...
			// alert(id);
			 if(id){
				 popupForm("<?=site_url($module_url)?>" + '/del/' + id + '/?width='+750+'&height='+525);
			 }else{
				 alert('Pilih baris yang ingin diedit');
			 }
		   }, 
