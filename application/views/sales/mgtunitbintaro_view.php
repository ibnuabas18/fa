...
<div class="classhome"><a href="<?=site_url('sales/viewmgt_status')?>" ><< back</a></div>
		<?php 
			...
			$no = 1;
			#die($p);
			$hasil = $this->db->query("Viewunitmgt ".$pp."")->result();
			...
			foreach($hasil as $row):
				...
				if($no==$max){
					echo "<td bgcolor=".$bgcolor." style='height:24px'><a class='thickbox' href='form_mgt_bintaro/".$row->unit_id."?width=800&height=500' title='Reserved Unit'>".$row->unit_no."</a></td>";
					echo"<tr>";
					$no = 0;
				}else{
					
					echo "<td bgcolor=".$bgcolor."><a class='thickbox' href='form_mgt_bintaro/".$row->unit_id."?width=800&height=500' title='Reserved Unit'>".$row->unit_no."</a></td>";
		//	
