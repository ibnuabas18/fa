...
$(document).ready(function(){
		...
		$('.calculate').bind('keyup keypress',function(){
			...
			var discount = parseInt($('#disc1').val());
			var adddisc = parseInt($('#adddisc').val());
			var phase = parseInt($('#discountplan').val());
			var adddiscplan = parseInt($('#adddiscplan').val());
					if (discount > phase) {
					
                              alert('Nilai Discount lebih besar');
							  $('#disc1').val(0);
							}
					else if (adddisc > adddiscplan) {
										//	if (grand_total3 > total3) {
					
                              alert('Nilai AddDisc lebih besar');
							  $('#adddisc').val(0);
							}
							});	
	...
	$('#discount').change(function(){
		...
		//	$('#paytipe').show();
			if($('#salesname option:selected').val() == 31108112){				
					$('.agenhide').show();
				}
				else {
					$('.agenhide').hide();
	...
	$('#subproject').change(function(){
			...
			$.getJSON('<?=site_url('spcustomer/salesplan')?>/'+$(this).val(),
					function(salesplan){
							$('#discountplan').val(salesplan.discount);
							$('#adddiscplan').val(salesplan.adddisc);
						})
	...
	$('#paytipe').change(function(){
				...
				var pricelistbdm = $('#pricehid').val();
				var discount = $('#disc1').val();
				var adddisc = $('#adddisc').val();
				var luasbdm = $('#luasbdm').val();
				...
				$.getJSON('<?=site_url('spcustomer/pricelistbdm')?>/'+$(this).val()+'/'+pricelistbdm+'/'+discount+'/'+adddisc+'/'+luasbdm,
					function(pricelistbdm){
							$('#soldprice').val(numToCurr(pricelistbdm.selling_price));
							$('#taxamount').val(numToCurr(pricelistbdm.tax));
							$('#payment').val(numToCurr(pricelistbdm.luasbdm));
						})
...
<form method="post" action="<?=site_url()?>spcustomer/insertcustomerbdm" id="formadd">
...
<div style="border:1px solid gray; width:750px; margin-bottom: 1em; padding: 10px">
    ...
    <!--Customer Profile-->
		...
		<tr>
			...
			<td>:</td>
			<td><input type="text" name="npwp" id="npwp" class="xinput" ></td>
		</tr>
	...
	<div id="country2" class="tabcontent">
			...
			<input type='hidden' name='hariini' id='hariini' value="<?=$tgl?>">
			 ...
			 <tr class="sphide">
				...
				<td>:</td>
				<?php $tgl = date("d-m-Y"); ?>
				<td><input style='width:100px' type="text" name="spdate" id="" value="<?=$tgl?>" class="xinput validate[required] datepicker" readonly="true" />
				<!--<td><input style='width:100px' type="text" name="spdate" id="" value="<?=$tgl?>" readonly="true" />-->
				...
				<td ><input type="hidden" name="discountplan" id="discountplan" class="xinput validate[required]"  style='width:50px' readonly>
				<input type="hidden" name="adddiscplan" id="adddiscplan" class="xinput validate[required]"  style='width:50px' readonly>
				<input type="hidden" name="luasbdm" id="luasbdm" class="xinput validate[required]"  style='width:50px' readonly>
				</td>
				<tr class="sphide">
					<td>SP No*</td>
					<td>:</td>
					<td ><input type="text" name="spno" id="spno" class="xinput validate[required]"  style='width:50px' readonly></td>
						...
						<td>:</td>
						<td colspan='5'><input type="number" name="disc1" id="disc1"  style='width:50px'  maxlength='5' ><span class="calculate hide_x"></span><input type="hidden" name="disc2" id="disc2" class="calculate" style='width:50px' maxlength='5'>
						<span class="hide_x">Add Disc</span>
						<input type="number" step="any" name="adddisc" id="adddisc" align="right" style='width:50px' maxlength='5' >
						</td>
				</tr>
				<tr class="sphide">
					<td>Project*</td>
				...
				<tr class="agenhide">
					...
					<td>					
						...
						<td>Agen Name</td>
						<td>:</td>
						<td colspan='5'><input type name="salesagen"  id="salesagen" />
							<option value=0></option></select>
						</td>
				</tr>
			</tr>
				...
				<tr class="sphide">
					...
					<td><span id='unitno'></span></td>
						<td>Down Payment</td>
							<td>:</td>
							<td class="hide_z">
								<input type="text" name="paytipedp"  id="paytipedp" class="input calculate"  style="width:50px">
								</select>X
							</td>
								<td class="hide_z">
								<input type="text" name="persendp"  id="persendp" class="input calculate"  style="width:50px">
								</select>%
							</td>
							<td colspan='3' class="hide_z">
				...
				<tr>
					...
					<td><font color='red'><span id='unitstatus'></font></td>
							...
							<td>:</td>
							<td class="hide_z">
								<input type="text" name="paytipepl"  id="paytipepl" class="input calculate" style='width:50px' />
								</select>X
							</td>
							<td></td>
							<td colspan='3' class="hide_z">
				...
				<tr>
					<td>Price Manual</td>
					<td>:</td>
					<td><input type="text" name="mprice" id="mprice" class="input validate[required] calculate  hide_g" maxlength="13" value = '0' align="right"/></td>
