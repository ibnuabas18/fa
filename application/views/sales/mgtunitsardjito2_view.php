...
<div class="classhome"><a href="<?=site_url('sales/viewmgt_status')?>" ><< back</a></div>
		<?php 
			$max = 34;
			$no = 1;
			$hasil = $this->db->query("Viewunit")->result();
			...
			foreach($hasil as $row):
				...
				if($no==$max){
					echo "<td bgcolor=".$bgcolor."><a class='thickbox' href='form_mgt_sardjito/".$row->unit_id."?width=800&height=500' title='Reserved Unit'>".$row->unit_no."</a></td>";
					echo"<tr>";
					$no = 0;
				}else{
