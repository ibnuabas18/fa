...
//~ });	
 ...
 function loadData(type,parentId){
      ...
      $.post('<?=site_url('sales/paycustomerlease/loaddata')?>', //request ke fungsi load data di inputAP
...
$(function(){
	...
	$(document).ready(function(){
		...
		function(){
			...
			$.getJSON('<?=site_url('sales/paycustomerlease/cekbilling')?>/'+$(this).val()+'/'+proj,
			function(data){
				//tampilbill(data.unit_id,data.customer_id)
				$('#customername').val(data.trade_name);
...
<div class="printed">
<form method="post" action="<?=base_url()?>sales/paycustomerlease/tampilbill" onsubmit="target_popup(this)">
...
<div class="printed">
<form method="post" id="myform" action="<?=base_url()?>sales/paycustomerlease/bayar" onsubmit="target_popup(this)">
<input type="hidden" name="customerid" id="customerid"/>
<table>		
		<tr id='frame10'>
		<td>Project</td>
	...
	<tr id='frame11'>
		<td>KD Tenant</td>
			<td>:</td>
				<td><input name='unit1' value='<?=@$data['roow']->kd_tenant?>' class='xinput'></select></td>
...
//var_dump($cek);
	...
	if($row->id_flag == 1){
		$status = "<a id='link' class='thickbox' title='Bayar bill' href='".site_url('sales/paycustomerlease/bayarbill')."/".$billing."/".$customer."/".$bayar."?width=750&height=450'/>Pay</a>";
		$color ="#FFF";
	...
	?>
		...
		<td style='width:150px' align="center"><input name='invoice<?=$no?>' id='invoice<?=$no?>' value=<?php echo $row->no_invoice ?> style='text-align:right' readonly></td>
		<!--<td style='width:150px' align="center"><input name='descs<?=$no?>' id='descs<?=$no?>' value=<?($row->descs)?> readonly></td>-->
