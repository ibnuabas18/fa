<?php
			...
			$data1 = $this->db->query("PaymentCustomerReport_leasing '".$subproject."','".$start_date."','".$end_date."',".$pt."");
			...
			$pdf->Cell(20,6,'LOO Date',1,0,'C',1);
			$pdf->Cell(50,6,'Customer',1,0,'C',1);
			$pdf->Cell(20,6,'Unit.No',1,0,'C',1);
			$pdf->Cell(25,6,'TOP',1,0,'C',1);
			$pdf->Cell(35,6,'Contract Price',1,0,'C',1);
			$pdf->Cell(32,6,'Total Billing',1,0,'C',1);
			$pdf->Cell(32,6,'Payment',1,0,'C',1);
			// $pdf->Cell(22,6,'Payment DP',1,0,'C',1);
			// $pdf->Cell(22,6,'Payment PL',1,0,'C',1);
			$pdf->Cell(32,6,'O/S',1,0,'C',1);
			$pdf->Cell(18,6,'Paid',1,0,'C',1);
			...
			foreach($data as $row){
				...
				if($no == $max){
						...
						#END
							...
							$pdf->Cell(25,6,'TOP',1,0,'C',1);
							$pdf->Cell(35,6,'Contract Price',1,0,'C',1);
							$pdf->Cell(32,6,'Total Paid',1,0,'C',1);
							$pdf->Cell(32,6,'Payment',1,0,'C',1);
							// $pdf->Cell(22,6,'Payment DP',1,0,'C',1);
							// $pdf->Cell(22,6,'Payment PL',1,0,'C',1);
							$pdf->Cell(32,6,'O/S',1,0,'C',1);
			...
			$selling = 	$row->billing;
			$os = $selling - $bf;
			$persen = ($bf/$selling) * 100;
			
			//Total Keseluruhan Payment
			$grandtotpaid = $grandtotpaid + $totpaid;
			$grandtotbf = $grandtotbf + $bf;
			$grandtotdp = $grandtotdp + $totpaid;
			...
			$pdf->Cell(50,8,$customernama,20,0,'L');
			$pdf->Cell(20,8,$unitno,20,0,'L');
			$pdf->Cell(26,8,$row->nama,20,0,'C');
			#$pdf->Cell(10,8,$tanah,20,0,'C');
			#$pdf->Cell(10,8,$bangunan,20,0,'C');
			$pdf->Cell(35,8,number_format($totpaid),20,0,'R');
			$pdf->Cell(32,8,number_format($totpaid),20,0,'R');
			$pdf->Cell(32,8,number_format($bf),20,0,'R');
			// $pdf->Cell(22,8,number_format($dp),20,0,'R');
			// $pdf->Cell(22,8,number_format($pl),20,0,'R');
			$pdf->Cell(32,8,number_format($os),20,0,'R');
			$pdf->Cell(18,8,number_format($persen).'%',20,0,'C');
			...
			$pdf->Ln();
					...
					$pdf->Cell(123,8,'TOTAL',0,0,'R',1);
					#$pdf->Cell(25,8,number_format($totpricemanual),0,0,'R',1);
					#$pdf->Cell(19,8,number_format($totdiscamount),0,0,'R',1);
					$pdf->Cell(35,8,number_format($totsellingprice),0,0,'R',1);
					#$pdf->Cell(20,8,'',0,0,'R',1);
					$pdf->Cell(32,8,number_format($grandtotpaid),0,0,'R',1);
					$pdf->Cell(32,8,number_format($grandtotbf),0,0,'R',1);
					// $pdf->Cell(22,8,number_format($grandtotdp),0,0,'R',1);
					// $pdf->Cell(22,8,number_format($grandtotpl),0,0,'R',1);
					$pdf->Cell(32,8,number_format($grandos),0,0,'R',1);
