<?php
	...
	$pdf->Cell(20,6,'Voucher No',1,0,'C',0);
	$pdf->Cell(20,6,'Kwitansi No',1,0,'C',0);
	$pdf->Cell(30,6,'Date',1,0,'C',0);
	#$pdf->Cell(40,6,'SP No',1,0,'C',0);
	$pdf->Cell(100,6,'Description',1,0,'C',0);
	$pdf->Cell(30,6,'Stored Bank',1,0,'C',0);
	$pdf->Cell(30,6,'Amount',1,0,'C',0);
	$pdf->Ln();				
	
	#Cek Data Payment
	$data = $this->db->query("SP_history_payment_leasing ".$proj.",".$pt.",'".$start."','".$end."'")->result();
	...
	foreach($data as $row){
		...
		$pdf->Cell(20,6,'GV'.$row->kwtbill_no,10,0,'C',0);
		$pdf->Cell(20,6,'KG'.$row->kwtbill_no,10,0,'C',0);
		$pdf->Cell(30,6,indo_date($row->kwtbill_paydate),10,0,'C',0);		
		#$pdf->Cell(40,6,$row->no_sp.'/SP/AC-YOGYA/GMI',10,0,'C',0);
		$pdf->Cell(100,6,$row->kwtbill_remark,10,0,'L',0);
