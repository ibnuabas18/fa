<?php
	//die($idnama);
			...
			$rows = $this->db->query("sp_print_suratpesanan_bintaro '".$idnama."'")->row();#$this->db->query($sql)->row();
			//die($rows->sp_id);
			...
			$sk = 'No. : '.$nosp.'/SP/'.$kode.'-BINTARO/ALK';
	...
	$pdf->MultiCell(200,0,$text,10,0,'L');	
			...
			if($idproject == 41011){	
				...
				$pdf->Cell(40,0,'sebagaimana ketentuan SURAT PESANAN  (terlampir).',10,0,'L');}
							...
							$pdf->Cell(40,0,'dengan ini menyatakan sepakat dan setuju untuk memesan '.$tipeproject.' di '.$project.' ("'.$tipeprojdet.'") dari PT. AKASA ',10,0,'L');	
			
							#end header
							$pdf->Ln(4);
							$pdf->Cell(40,0,'LEGIAN KARYA  yang berkedudukan  di Jakarta  ("PENERIMA PESANAN")  dan   PEMESAN   wajib  tunduk  pada Syarat-syarat dan  Ketentuan-',10,0,'L');	
			...
			if($idproject == 41013){	
			$pdf->Ln(7);
			$pdf->Cell(40,0,$tipeproject.' yang  dipesan oleh  PEMESAN  yang terletak di  Kelurahan  Pondok Jaya, Kecamatan  Pondok Aren,  Propinsi ',10,0,'L');	
			}else{
			$pdf->Ln(7);
			$pdf->Cell(40,0,$tipeproject.' yang  dipesan oleh  PEMESAN  yang terletak di  Kelurahan  Pondok Jaya, Kecamatan  Pondok Aren,  Tangerang Selatan, ',10,0,'L');	
			}
			$pdf->Ln(4);
			$pdf->Cell(40,0,'Banten,  dengan uraian sebagai berikut :',10,0,'L');	
			...
			if($idproject == 41011){
						...
						$pdf->Cell(23,0,$luas.' M2 / '.$luasbangun.' M2',10,0,'L');}
									...
									$pdf->Cell(45,0,'Lantai / Tipe',10,0,'L');
									$pdf->Cell(9,0,':',10,0,'L');
									$pdf->Cell(6,0,$lantai,10,0,'L');
									$pdf->Cell(6,0,'/',5,0,'L');
									$pdf->Cell(6,0,$Tipe,5,0,'L');
		...
		$pdf->Ln(4);
			...
			$pdf->Ln(7);
			$pdf->Cell(55,6,'Rekening PENERIMA PESANAN :',5,0,'L',0);
			$pdf->Cell(20,6,'PT.Akasa Legian Karya',5,0,'L',0);
			
			$pdf->Ln(4);
			$pdf->Cell(55,6,'',10,0,'L',0);
			$pdf->Cell(20,6,'Bank',5,0,'L',0);
			$pdf->Cell(12,6,':',5,0,'L',0);
			$pdf->Cell(12,6,$dtbank->bank_nm,5,0,'L',0);
			...
			if($idproject == 41011){
					...
					$pdf->Cell(55,6,'sejak tanggal Pelunasan UANG MUKA ("TANGGAL SELESAI PEMBANGUNAN")',10,0,'L',0);}
						elseif  ($idproject == 41013){
								...
								$pdf->Cell(55,6,'PENERIMA  PESANAN  dengan   ini  berjanji  dan  mengikatkan  diri untuk  menyelesaikan  pembangunan    UNIT  APARTEMEN   pada  tanggal  ',10,0,'L',0);
								$pdf->Ln(4);
								$pdf->Cell(55,6,'30 Nopember 2017 ("TANGGAL SELESAI PEMBANGUNAN").',10,0,'L',0);
			...
			$pdf->Ln(4);
			$pdf->Cell(60,6,'PT. Akasa Legian Karya',10,0,'L',0);
			...
			$pdf->Cell(60,6,substr($nama,0,27),10,0,'L',0);
			$pdf->Cell(60,6,$sales,10,0,'L',0);
			}else{
			$pdf->Cell(60,6,'Penerima Pesanan',10,0,'L',0);
			$pdf->Cell(60,6,'PEMESAN',10,0,'L',0);
			$pdf->Cell(60,6,'Agen',10,0,'L',0);
			$pdf->Ln(4);
			$pdf->Cell(60,6,'PT. Akasa Legian Karya',10,0,'L',0);
