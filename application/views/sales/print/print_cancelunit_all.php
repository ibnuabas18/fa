<?php
			...
			$sql = 	"select unit_no, tgl_cancel, customer_nama, '0' as charges, descs, nm_subproject from db_historyunit
							inner join db_unit_bdm on unit_id=id_unit
							inner join db_customer on id_customer=customer_id
							inner join db_subproject on subproject_id=db_historyunit.id_subproject
							where db_historyunit.id_subproject=$proj
							ORDER BY tgl_cancel DESC";
			$rw = $this->db->query($sql)->result();
			
			$rows = 	"select unit_no, tgl_cancel, customer_nama, '0' as charges, descs, nm_subproject from db_historyunit
							inner join db_unit_bdm on unit_id=id_unit
							inner join db_customer on id_customer=customer_id
							inner join db_subproject on subproject_id=db_historyunit.id_subproject
							where db_historyunit.id_subproject=$proj
							ORDER BY tgl_cancel DESC";
			$hasil = $this->db->query($rows)->row();
			...
			#Header
				...
				$pdf->SetX(25);
				$pdf->Cell(0,10,@$hasil->nm_subproject,20,0,'L');
			...
			//	$pdf->Cell(0,10,'PROJECT : '.@$rw->nm_subproject,20,0,'L');
				$pdf->SetXY(25,26);
				//$pdf->Cell(10,10,'AS OF : '.$tgl,20,0,'L');
			...
			$pdf->Cell(125,5,'Cancel Date',1,0,'C',1);
			//~ $pdf->Cell(15,5,'Mail Post',1,0,'C',1);
			$pdf->Cell(20,5,'Charges',1,0,'C',1);
			//~ $pdf->Cell(15,5,'Fax',1,0,'C',1);
			//~ $pdf->Cell(10,5,'Umur',1,0,'C',1);
			$pdf->Cell(35,5,'Reason',1,0,'C',1);
			//~ 
	...
	foreach ($rw as $row){
		if($noo == $max){ 
			...
			#Header
				...
				$pdf->SetX(25);
				$pdf->Cell(0,10,$row->nm_subproject,20,0,'L');
	...
	$a = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa4";
		//~ $tgl_lhr = indo_date($row->customer_tgl_lhr);
			...
			$pdf->Cell(125,5,indo_date($row->tgl_cancel),1,0,'L',0);
			//~ $pdf->Cell(15,5,$row->kdpos1,1,0,'C',0);
			$pdf->Cell(20,5,$row->charges,1,0,'L',0);
			//~ $pdf->Cell(15,5,$row->customer_fax,1,0,'L',0);
			//~ $pdf->Cell(10,5,$row->umur,1,0,'C',0);
			$pdf->Cell(35,5,$row->descs,1,0,'L',0);
		//~ 
			$pdf->Ln(5);		
			$i++;
			$no++;
