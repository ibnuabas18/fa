<?php
	//die($idnama);
			...
			$rows = $this->db->query("sp_print_suratpesanan '".$idnama."'")->row();#$this->db->query($sql)->row();
			//die($rows->sp_id);
			...
			if($idproject == 41011 || $idproject == 41014){ $tipeproject ="UNIT RUMAH";
										$tipeprojdet ="RUMAH";
										$tempat ="Perumahan";
			...
			//$title2 = 'SURAT PESANAN UNIT  '.$unit.' / '.$unit_nm.' ("'.$tipeproject.'")';
			$sk = 'No. : '.$nosp.'/SP/'.$kode.'-YOGYA/GMI';
	...
	$pdf->MultiCell(200,0,$text,10,0,'L');	
			...
			if($idproject == 41011 || $idproject == 41014){	
				$pdf->Ln(4);
				$pdf->Cell(40,0,'dengan ini menyatakan sepakat dan setuju untuk  memesan '.$tipeproject.'  di '.$project.' ("'.$tipeprojdet.'")dari PT. GRAHA MULTI INSANI  ',10,0,'L');	
				//$pdf->Cell(40,0,'dengan ini menyatakan sepakat dan setuju untuk  memesan '.$tipeproject.'  di '.$project.' ("'.$tipeprojdet.'") dari PT. GRAHA MULTI',10,0,'L');	
				...
				//$pdf->Cell(40,0,'INSANI yang berkedudukan di Jakarta ("PENERIMA PESANAN") dan PEMESAN  wajib tunduk  pada Syarat-syarat dan  Ketentuan-ketentuan Pemesanan  ',10,0,'L');	
				...
				$pdf->Cell(40,0,'sebagaimana ketentuan SURAT PESANAN  (terlampir).',10,0,'L');}
							...
							#end header
							$pdf->Ln(4);
							$pdf->Cell(40,0,'INSANI  yang berkedudukan  di Jakarta  ("PENERIMA PESANAN")  dan   PEMESAN   wajib  tunduk  pada Syarat-syarat dan  Ketentuan-ketentuan  ',10,0,'L');	
			...
			if($idproject == 41013){	
			$pdf->Ln(7);
			$pdf->Cell(40,0,$tipeproject.' yang  dipesan oleh  PEMESAN  yang terletak di  Kelurahan  Terban, Kecamatan  Gondokusuman Yogyakarta,  Propinsi ',10,0,'L');	
			}else{
			$pdf->Ln(7);
			$pdf->Cell(40,0,$tipeproject.' yang  dipesan oleh  PEMESAN  yang terletak di  Kelurahan  Mantrijeron, Kecamatan  Mantrijeron, Kota  Yogyakarta,  Propinsi ',10,0,'L');	
			}
			$pdf->Ln(4);
			$pdf->Cell(40,0,'Daerah Istimewa Yogyakarta dengan uraian sebagai berikut :',10,0,'L');	
		...
		$pdf->Ln(4);
			...
			if($idproject == 41011 || $idproject == 41014){
					...
					$pdf->Cell(55,6,'sejak tanggal Pelunasan UANG MUKA ("TANGGAL SELESAI PEMBANGUNAN")',10,0,'L',0);}
						elseif  ($idproject == 41013){
								...
								$pdf->Ln(4);
								$pdf->Cell(55,6,'31 Oktober 2015 ("TANGGAL SELESAI PEMBANGUNAN").',10,0,'L',0);
								}else{
								$pdf->Ln(4);
								$pdf->Cell(55,6,'PENERIMA  PESANAN  dengan   ini  berjanji  dan  mengikatkan  diri untuk  menyelesaikan  pembangunan    UNIT  APARTEMEN   pada  tanggal  ',10,0,'L',0);
								$pdf->Ln(4);
								$pdf->Cell(55,6,'31 Juli 2014 ("TANGGAL SELESAI PEMBANGUNAN").',10,0,'L',0);
			...
			$pdf->Ln(4);
			$pdf->Cell(60,6,'PT. GRAHA MULTI INSANI',10,0,'L',0);
			...
			$pdf->Cell(60,6,substr($nama,0,28),10,0,'L',0);
			$pdf->Cell(60,6,$sales,10,0,'L',0);
			...
			$pdf->Cell(60,6,substr($nama,28,28),10,0,'L',0);
