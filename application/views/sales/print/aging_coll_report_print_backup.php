<?php
	...
	#PROJECT NAME
	if($subproject == 41011){ $project = 'AWANA TOWN HOUSE';}
	elseif($subproject == 41012){ $project = 'AWANA CONDOTEL';}
	elseif($subproject == 11101){ $project = 'Taman Rasuna';}
	elseif($subproject == 1103){ $project = 'Rasuna Office Park Tahap 1';}
	elseif($subproject == 1104){ $project = 'Rasuna Office Park Tahap 2';}
	elseif($subproject == 11102){ $project = 'Aston Rasuna Residence - Serviced Apartement';}
	elseif($subproject == 1110){ $project = 'The 18TH Rasuna Res. South Tower';}
	elseif($subproject == 1105){ $project = 'Rasuna Office Park Tahap 3';}
	elseif($subproject == 11203){ $project = 'Rasuna Epicentrum (Lifestyle Office)';}
	...
	$pdf->Cell(10,4,'Unit No',1,0,'C',0);
	$pdf->Cell(30,4,'Customer Name',1,0,'C',0);
	$pdf->Cell(15,4,'Payment Type',1,0,'C',0);
	$pdf->Cell(12,4,'Sales Date',1,0,'C',0);
	$pdf->Cell(12,4,'Sqm.Nett',1,0,'C',0);
	$pdf->Cell(12,4,'Sqm.SGA',1,0,'C',0);
	$pdf->Cell(20,4,'Selling Price (Ppn)',1,0,'C',0);
	...
	// $sql = $this->db->select('datediff(day, due_date,getdate()) as jumlahhari,id_billing')
							...
							// ->result();
	$sql = $this->db->select('datediff(day, due_date,getdate()) as jumlahhari,id_billing')
							->join('db_kwtbill','id_bill  = id_billing')
							->where('id_sp',$idsp)
							->where('kwtbill_paydate <=',$start_date)
							->where('isnull(db_billing.id_flag,0) !=',10)
							->get('db_billing')
							->result();						
	...
	#var_dump($query);
			...
			$persen	= ($payamount/$sellprice)*100;
					...
					$totcoll 		= $totcoll + ($sql->payamount);
					$totnotdue 		= $totnotdue + $notdue;
					$totaging30		= $totaging30 + $aging30;
					$totaging60 	= $totaging60 + $aging60;
					$totaging90 	= $totaging90 + $aging90;
					$totagingover 	= $totagingover + $agingover;
					$totoverdue		=  $totaging30 + $totaging60 + $totaging90 + $totagingover ;
					
					
			#OVERDUE
			$overdue = $aging30 + $aging60 + $aging90 + $agingover;
			...
			$pdf->Cell(10,4,$row->unit_no,0,0,'L');
			$pdf->Cell(30,4,$row->customer_nama,0,0,'L');
			...
			$pdf->Cell(20,4,number_format($sql->payamount),0,0,'R');
			$pdf->Cell(5,4,number_format($persen),0,0,'R');
			$pdf->Cell(20,4,number_format($notdue),0,0,'R');
			$pdf->Cell(20,4,number_format($overdue),0,0,'R');
			$pdf->Cell(20,4,number_format($aging30),0,0,'R');
			$pdf->Cell(20,4,number_format($aging60),0,0,'R');
			$pdf->Cell(20,4,number_format($aging90),0,0,'R');
			$pdf->Cell(20,4,number_format($agingover),0,0,'R');
			
			#TOTAL OS
			$os = ($row->selling_price) - ($sql->payamount);
