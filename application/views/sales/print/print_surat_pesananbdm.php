<?php
	//die($idnama);
			...
			}elseif ($idproject==3105 || $idproject==3106){
			$lantai = 'GF';
			}else{
			$lantai = substr($unit,4,2);
			}
			$blok = substr($unit,6,3);
			#var_dump($lantai);
			...
			$wave = array('3101','3103');
			$ocea = '3102';
			$suite = array('3105','3106');
			// //$lima = array('3102','11102','11202','11214','11204','11224','11203');
			// //$delapan = array('1103','1104','1105');
			
			if ($idproject == $wave){
			
			 $sk = 'No. : '.substr($nosp,6,4).'/SP/TW-RE/BDM';
			 }elseif ($idproject == $ocea){
			
			 $sk = 'No. : '.substr($nosp,7,4).'/SP/OCEA/BDM';
			}else{
			$sk = 'No. : '.$nosp.'/SP/TW-RE/BDM';
			}
			//$idsp			
			$pdf->SetFont('Arial','B',12);
			$pdf->SetXY(2,16);
			...
			if ($idproject == 3106 || $idproject == 3105){
			$pdf->SetXY(70,22);
			...
			$pdf->Cell(9,0,':',10,0,'L');
			$pdf->Cell(9,0,substr($alamat1,0,90),10,0,'L');		
						
			$pdf->Ln(4);
			$pdf->SetX(52);
			$pdf->Cell(9,0,substr($alamat1,90,90),10,0,'L');			
			$pdf->Ln(4);
			...
			$pdf->Cell(9,0,substr($alamat2,0,90),10,0,'L');		
			$pdf->Ln(4);
			$pdf->SetX(52);
			$pdf->Cell(9,0,substr($alamat2,90,90),10,0,'L');	
	...
	$pdf->MultiCell(200,0,$text,10,0,'L');	
			...
			if($idproject == 41011){	
				$pdf->Ln(4);
				$pdf->Cell(40,0,'dengan ini menyatakan sepakat dan setuju untuk  memesan '.$tipeproject.'  di '.$project.' ("'.$tipeprojdet.'")dari PT. BUMI DAYA MAKMUR  ',10,0,'L');	
				...
				$pdf->Cell(40,0,'sebagaimana ketentuan SURAT PESANAN  (terlampir).',10,0,'L');}
							...
							$pdf->Cell(40,0,'dengan ini menyatakan sepakat dan setuju untuk memesan '.$tipeproject.' di '.$project.' ("'.$tipeprojdet.'") dari PT. BUMI',10,0,'L');	
			
							#end header
							$pdf->Ln(4);
							$pdf->Cell(40,0,'DAYA MAKMUR  yang berkedudukan  di Jakarta  ("PENERIMA PESANAN")  dan   PEMESAN   wajib  tunduk  pada Syarat-syarat ',10,0,'L');	
			
							$pdf->Ln(4);
							$pdf->Cell(40,0,'dan  Ketentuan-ketentuan Pemesanan sebagaimana ketentuan SURAT PESANAN  (terlampir).',10,0,'L');}
			...
			if($idproject == 41011){
						...
						$pdf->Cell(23,0,$luas.' M2 / '.$luasbangun.' M2',10,0,'L');}
									...
									// $pdf->Cell(20,0,'Blok',10,0,'L');
									// $pdf->Cell(4,0,':',10,0,'L');
									// $pdf->Cell(4,0,$blok,10,0,'L');
								}
			$pdf->Ln(4);
			if($idproject == 41011){
		...
		$pdf->Ln(2);
			...
			$pdf->SetFont('Arial','',5);
			$pdf->Cell(45,6,'('.ucwords(toRupiah($rows->bf)).' )',10,0,'L',0);
			$pdf->SetFont('Arial','',9);
		...
		$pdf->Ln(4);
			...
			if($idproject == 41011){
					...
					$pdf->Cell(55,6,'sejak tanggal Pelunasan UANG MUKA ("TANGGAL SELESAI PEMBANGUNAN")',10,0,'L',0);}
								...
								}elseif ($idproject == 3101 || $idproject == 3105){
								$pdf->Cell(55,6,'30 Juli 2013 ("TANGGAL SELESAI PEMBANGUNAN").',10,0,'L',0);
								}else{
								$pdf->Cell(55,6,'30 Juni 2014 ("TANGGAL SELESAI PEMBANGUNAN").',10,0,'L',0);
								}
								}
			...
			$pdf->Ln(4);
			$pdf->Cell(60,6,'PT. BUMI DAYA MAKMUR',10,0,'L',0);
