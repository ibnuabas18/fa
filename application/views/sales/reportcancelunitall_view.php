...
function loadData(type,parentId){
      ...
      $.post('<?=site_url('reportcancelunitall/loaddata')?>', //request ke fungsi load data di inputAP
	...
	$('#subproject').change( 
		...
		function(){
			...
			//~ if($('#tipe option:selected').val() != '')
				...
				$.getJSON('<?=site_url('reportcancelunitall/gettype')?>/'+ a,
							function(data){
								$('#tipe').empty(); 
...
<h2><font color='red' size='4'>Report Cancel Unit<hr width="150px" align="left"></font></h2>
<div class="printed">
<form method="post" action="<?=site_url('reportcancelunitall/print_cancelunitall')?>" target="_blank">
