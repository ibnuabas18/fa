<?php
			...
			$rows = $this->db->query('sp_print_suratpesanan')->row();#$this->db->query($sql)->row();
			...
			$title1 = "THE GROVE SUITES";
			$title2 = 'SURAT PESANAN UNIT '.$unit.' ("UNIT APARTEMEN")';
			$sk = 'No. : 0216/SP/GV-RE/BSU ("UNIT APARTEMEN")';
			...
			$pdf->Ln(12);
			$pdf->Cell(40,0,'No. Identitas',10,0,'L');
			$pdf->Cell(9,0,':',10,0,'L');
			$pdf->Cell(9,0,$identitas,10,0,'L');
			$pdf->Ln(4);
			$pdf->Cell(40,0,'Telepon / HP',10,0,'L');
			$pdf->Cell(9,0,':',10,0,'L');
			$pdf->Cell(9,0,$tlp."/".$hp,10,0,'L');
			$pdf->Ln(4);
			$pdf->Cell(40,0,'Alamat Surat',10,0,'L');
			$pdf->Cell(9,0,':',10,0,'L');
			...
			$pdf->SetFont('Arial','',8);
			$pdf->Ln(12);
			$pdf->Cell(30,0,'(selanjutnya disebut "',10,0,'L');
			$pdf->SetFont('Arial','B',8);
			$pdf->Cell(15,0,$a,10,0,'L');
			$pdf->SetFont('Arial','',7);
			$pdf->Cell(10,0,'" )',10,0,'L');
			/*
			$text = '
	...
	$pdf->MultiCell(200,0,$text,10,0,'L');	
			...
			$pdf->Ln(4);
			$pdf->Cell(40,0,'dengan ini menyatakan sepakat dan setuju untuk memesan UNIT APARTEMEN di THE GROVE SUITES ("APARTEMENT")',10,0,'L');	
			#end header
			$pdf->Ln(4);
			$pdf->Cell(40,0,'dari PT. BAKRIE SWASAKTI UTAMA yang berkedudukan di Jakarta ("PENERIMA PESANAN") dan PEMESAN wajib tunduk',10,0,'L');	
			
			$pdf->Ln(4);
			$pdf->Cell(40,0,'pada Syarat-syarat dan Ketentuan-ketentuan pemesanan sebagaimana ketentuan SURAT PESANAN (terlampir).',10,0,'L');	
			
			$pdf->Ln(7);
			$pdf->Cell(40,0,'UNIT APARTEMEN yang dipesan oleh PEMESAN yang terletak di Kel. Karet Kuningan, Kec. Setiabudi, Kotamadya Jakarta Selatan dengan',10,0,'L');	
			$pdf->Ln(4);
			$pdf->Cell(40,0,'uraian sebagai berikut :',10,0,'L');	
			
			$pdf->Ln(7);
			$pdf->Cell(55,0,'Peruntukan',10,0,'L');
			$pdf->Cell(9,0,':',10,0,'L');
			$pdf->Cell(9,0,'Hunian',10,0,'L');
			$pdf->Ln(4);
			$pdf->Cell(55,0,'Gedung/Menara/Tower',10,0,'L');
			$pdf->Cell(9,0,':',10,0,'L');
			$pdf->Cell(9,0,$project,10,0,'L');
			$pdf->Ln(4);
			// Lantai 
			#awal Lantai
			$pdf->Cell(55,0,'Lantai',10,0,'L');
			$pdf->Cell(9,0,':',10,0,'L');
			$pdf->Cell(15,0,$lantai,10,0,'L');
			$pdf->Cell(9,0,'Tipe :',10,0,'L');
			$pdf->Cell(60,0,$project,10,0,'L');
			$pdf->Cell(30,0,'Luas (Semi Gross) :',10,0,'L');
			$pdf->Cell(23,0,$luas,10,0,'L');
			#akhir Lantai
			
			$pdf->Ln(4);
			$pdf->Cell(55,0,'Harga Jual (Incl. PPN )',10,0,'L');
			$pdf->Cell(9,0,':',10,0,'L');
			$pdf->Cell(9,0,$hargajual,10,0,'L');
			
			#awal cara bayar
					$pdf->Ln(4);
			$pdf->Cell(55,0,'Cara Pembayaran',10,0,'L');
			$pdf->Cell(9,0,':',10,0,'L');
		...
		$pdf->Ln(4);
			...
			$pdf->Ln(7);
			$pdf->Cell(55,6,'Harga Jual termasuk biaya-biaya :',10,0,'L',0);
			
			$pdf->Ln(4);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(100,6,'Pajak Pertambahan Nilai ("PPN")',10,0,'L',0);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(12,6,'Jaringan air bersih',10,0,'L',0);
			
			$pdf->Ln(4);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(100,6,'Ijin Mendirikan Bangunan Induk ("IMB") induk',10,0,'L',0);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(12,6,'Jaringan telepon',10,0,'L',0);
			
			$pdf->Ln(4);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(100,6,'Ijin Layak Huni atau Ijin Penggunaan Bangunan ("IPB") induk',10,0,'L',0);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(12,6,'Jaringan listrik',10,0,'L',0);
			
			$pdf->Ln(4);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(100,6,'Pemecahan Sertifikat Hak Milik atas Satuan Rumah Susun ("SHMSRS")',10,0,'L',0);
			$pdf->Cell(4,6,'',10,0,'L',0);
			$pdf->Cell(12,6,'',10,0,'L',0);
			
			$pdf->Ln(7);
			$pdf->Cell(55,6,'Harga Jual atas UNIT APARTEMENT belum termasuk :',10,0,'L',0);
			
			
			$pdf->Ln(4);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(100,6,'Perjanjian Pengikatan Jual Beli ("PPJB")',10,0,'L',0);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(12,6,'Pajak Bumi & Bangunan ("PBB")',10,0,'L',0);
			
			$pdf->Ln(4);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(100,6,'Bea Perolehan Hak atas Tanah dan Bangunan ("BPHTB")',10,0,'L',0);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			...
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(12,6,'Biaya yang timbul berdasarkan Keputusan/peraturan dari pemerintah',10,0,'L',0);
			
			$pdf->Ln(4);
			$pdf->Cell(4,6,'-',10,0,'L',0);
			$pdf->Cell(100,6,'Biaya Balik nama SHMSRS ke atas nama PEMESAN',10,0,'L',0);
			$pdf->Cell(4,6,'',10,0,'L',0);
			$pdf->Cell(12,6,'(jika ada)',10,0,'L',0);
			
			$pdf->Ln(7);
			$pdf->Cell(55,6,'Dengan ditandatanganinya Surat Pesanan ini, maka PEMESAN mengakui dan menyetujui segala syarat-syarat, ketentuan-ketentuan yang ditetapkan',10,0,'L',0);
			
			$pdf->Ln(4);
			$pdf->Cell(55,6,'dalam Surat Pesanan ini.',10,0,'L',0);
			...
			$pdf->Cell(60,6,'Penerima Pesanan',10,0,'L',0);
			$pdf->Cell(60,6,$nama,10,0,'L',0);
			$pdf->Cell(60,6,$sales,10,0,'L',0);
			
			$pdf->Ln(4);
			$pdf->Cell(60,6,'Syarat-syarat & Ketentuan-ketentuan Pemesanan terlampir sebagai satu kesatuan dan bagian integral yang tidak terpisahkan dari SURAT PESANAN ini.',10,0,'L',0);
