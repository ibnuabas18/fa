...
$(function(){
	/* is profit checked here */
	$("#is_profit").click(function(){
		if ($("#is_profit").is(":checked")) {
			var c = confirm("Jadikan sebagai profit ?");
			if(!c){
				return false;
			}else{
				var id_subprojek = $(".get_projectid").val();
				var id_unit   = $(".get_unit").val();
				var amount = $(".get_amount").val();					
				$.ajax({
					url		:'<?php echo site_url();?>sales/paycustomerbsu/get_isprofit',
					type	:'post',
					data	:{'id_subprojek':id_subprojek,
							  'id_unit':id_unit,
							  'amount':amount
					},
					success	:function(data){
						alert(data);
						if(data=="noprofit"){
							var c = confirm("Pembayaran kurang dari 20%<br>, lanjutkan ?");
							if(!c){
								return false;
							}
						}
...
//~ });	
 ...
 function loadData(type,parentId){
      ...
      $.post('<?=site_url('sales/paycustomerbsu/loaddata')?>', //request ke fungsi load data di inputAP
...
$(function(){
	...
	$(document).ready(function(){
		...
		function(){
			...
			$.getJSON('<?=site_url('sales/paycustomerbsu/cekbilling')?>/'+$(this).val()+'/'+proj,
			function(data){
				//tampilbill(data.unit_id,data.customer_id)
				$('#customername').val(data.customer_nama);
				var nm = $('#customername').val().replace(/ /g,'-');
				$.getJSON('<?=site_url('sales/paycustomerbsu/getaps')?>/agus-susilo',
				function(getap){
				$('#id_vendor').val(getap.kd_supplier);
				var idv = $('#id_vendor').val();
				  //alert(getap.kd_supplier);
				if(idv!=''){
				
				$.post('<?=site_url('sales/paycustomerbsu/getnoap')?>/'+idv,{},function(obj){
					$('#list_ap').html(obj);
				});
				}
				});
			...
			//alert($('#customername').val(data.customer_nama));
		}
	);
	...
	$("#tipe").change(function(){
				if($("#tipe option:selected").val()=='Credit'){
					$("#charge").show();
					$("#terima1").show();
					$("#terima2").show();
					$('.list_apt').hide();
					$('.valapt').hide();
					$('.b').show();
				}else if($("#tipe option:selected").val()=='Set Off AP'){
					$('.list_apt').show();
					$('.valapt').show();
						$("#terima1").hide();
					$("#terima2").hide();
	...
	//loadData('unid',$('#sid option:selected').val());	
				if($("#bank option:selected").val()==54){
					loadData('unid',0);	 
					$(".sourcehide").show();
			
				}else if($("#bank option:selected").val()==56){
					loadData('unid',0);	 
					$(".sourcehide").show();
	...
	$('#unid').change(function(){
		var id = $('#unid').val();
		$.getJSON('<?=site_url('sales/paycustomerbsu/cek_data')?>/'+id,
			function(response){
...
$("#bank").change(function(){
	//$('#amount').val(0);
			$.getJSON('<?=site_url()?>/bankkeluar/getkodebank/' + $(this).val(),
			function(getdata){			
				$('#kd_bank').val(getdata.bank_acc);
				});
});
$('.list_apt').hide();
$('.valapt').hide();
$('#docno_ap').change(function(){
	$.getJSON('<?php echo base_url();?>/sales/paycustomerbsu/getvalap/'+$(this).val(),
	function(getval){
		$('#apamount').val(getval.base_amt);
		$('#ketap').val(getval.descs);
	});
});
$('#list_ap').change(function(){
	$.getJSON('<?php echo base_url();?>/sales/paycustomerbsu/getvalap/'+$(this).val(),
	function(getval){
		$('#valap').val(getval.base_amt);
	});
});
});
</script>
...
<script type="text/javascript">
	$(function(){
		/* ap-autocomplete */
		$( ".get_noap" ).autocomplete({
			source: function(request, response) {
					$.ajax({ url: "<?php echo site_url();?>autocomplete/get_appayment",
					data: { term: $(".get_noap").val()},
					dataType: "json",
					type: "POST",
					success: function(data){
						response(data);
...
<!-- add end here -->
<h2 id='frame0' style="margin-left:20px">Payment Customer<hr width="150px" align="left"></h2>
<div class="printed" style="margin-left:20px">
<form method="post" action="<?=base_url()?>sales/paycustomerbsu/tampilbill" onsubmit="target_popup(this)">
...
<table>
	<tr id='frame1'>
		<td>Project</td>
			<td>:</td>
				<td>
					<select name="subproject" id="subproject" class="xinput" style="width:157px"></select>
				</td>
		
		<td>Received Date</td>
			<td>:</td>
			<td><input type="text" name="tgl" id="tgl"  style="width:157px"></td>		
			<td class="list_apt">No AP</td>
			<td class="list_apt">:</td>
				<td class="list_apt"><select name="list_ap" id="list_ap">
				</select></td>
		<td class="b">Bank/Adj</td>
			<td class="b">:</td>
				<td class="b"><select name='bank' id='bank' style="width:150px"></td>
	...
	<tr id='frame2'>
		<td>Unit</td>
			<td>:</td>
				<td><select name='unit' id='unit' class='xinput' style="width:157px"></select></td>
		<td> Payment Type</td>
			<td>:</td>
				<td><select name='tipe' id='tipe' class='xinput' style="width:157px">
					<option></option>
					...
					<option value='Set Off AP'>Set Off AP</option>
					</select></td>
			<td class="valapt">Nilai AP</td>
			<td class="valapt">:</td>
				<td class="valapt"><input type="text" name="valap" id="valap"></td>
				
				<td class="b">Kode Bank</td>
			<td class="b">:</td>
				<td class="b"><input name='kd_bank' id='kd_bank' style="width:250px" readonly="readonly"></td>
		<!--<td>Komisi MKT</td>
			<td>:</td>
	...
	<tr id='frame3'>
		<td>Customer</td>
			<td>:</td>
				<td><input name='customername' id='customername' class='xinput' readonly  style="width:150px"/></td>
		
		<td>Amount</td>
			<td>:</td>
				<td><input name='amount' id='amount' class="input calculate validate[required]" style="width:150px"></td>
		
		
	<td id ="terima2">Charge</td>
			<td id ="terima1">:</td>
			<td><input type="text" name="charge" id="charge"  class="validate[required] xinput" value="0"  style="width:150px"/></td>
	...
	<tr id='frame4'>
		...
		<td>No.Reff</td>
			<td>:</td>
				<td><input name='reff' style="width:150px"></td>
	...
	<tr class="sourcehide">
		...
		<td>:</td>
		<td>
			<select name='unid' id="unid" class="xinput" style="width:150px"></select>
		</td>
		</tr>
...
<div class="printed">
<form method="post" id="myform" action="<?=base_url()?>sales/paycustomerbsu/new_bayar" onsubmit="">
<input type="hidden" name="customerid1" id="customerid1" value='<?=$data['customerid1']?>'/>
<table>		
		<tr id='frame10'>
		<td>Project</td>
			<td>:</td>
				<td>
					...
					<input type="hidden" value='<?=$data['row']->subproject_id?>' class="get_projectid">
				</td>
		...
		<td class="baj">Bank/Adj</td>
			<td class="baj">:</td>
				<td class="baj">
				<?php if($data['tipe_ap']=='Set Off AP'){?>
				<input name='bank1' value='<?php echo $data['bank_remark'];?>'>
				<?php }else{ ?>
				<input name='bank1' value='<?php echo $data['bank'];?>'>
				<?php } ?>
				<input type="hidden" name="kode_bank" value="<?php echo $data['kd_bank'];?>">
				</td>
				
		<?php if($data['tipe']=='Set Off AP'){?>
		<td>Nilai AP</td>
		<td>:</td>
		<td>
		<input type="text" value="<?php echo $data['valap'];?>">
		</td>
		<?php } ?>
		<td><!--Pengkuan 20%-->&nbsp;</td>
		<td>&nbsp;</td>
		<td><!--input name="is_profit" type="checkbox" id="is_profit"-->&nbsp;</td>
	...
	<tr id='frame11'>
		<td>Unit</td>
			<td>:</td>
				<td>
					<input name='unit1' value='<?=@$data['roow']->unit_no?>' class='xinput'></select>
					<input type="hidden" value='<?=@$data['roow']->unit_id?>' class='get_unit'></select>
				</td>
		<td>Payment Type</td>
			<td>:</td>
		...
		<td>No AP</td>
			<td>:</td>
				<td><input name='noaps' value='<?=$data['noaps']?>' class='xinput' readonly>	
		<?php }else{ ?>
		<td>Charge</td>
			<td>:</td>
				<td><input name='charge1' value='<?=$data['charge']?>' class='xinput' readonly>	
				<input type="hidden" name='unid1' value='<?=$data['unid1']?>'></td>		
		<?php } ?>
	...
	<tr id='frame12'>
		...
		<td>Amount</td>
			<td>:</td>
				<td><input name='amount1' id='amount1'value='<?=$data['amount']?>' class="get_amount" style='text-align:right' ></td>
		...
		<td>Komisi MKT</td>
			<td>:</td>
			<td>
				<input type="hidden" name="cekkomisi" id='cekkomisi' value="0" /> 
				<input type="checkbox" name="cekkomisi" id='cekkomisi' value="1" />
			</td>
	</tr>
	<tr id='frame13'>
		<td colspan='3'>&nbsp;</td>
...
//var_dump($cek);
	...
	if($row->id_flag == 1){
		$status = "<a id='link' class='thickbox' title='Bayar bill' href='".site_url('sales/paycustomerbsu/bayarbill')."/".$billing."/".$customer."/".$bayar."?width=750&height=450'/>Pay</a>";
		$color ="#FFF";
	...
	<input type="hidden" name="totrow" value="<?php echo $no; ?>">
