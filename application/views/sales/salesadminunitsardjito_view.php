...
<div class="classhome"><a href="<?=site_url('sales/viewsalesadmin_status')?>" ><< back</a></div>
		<?php 
			...
			$no = 1;
			#die($p);
			$hasil = $this->db->query("Viewunit ".$pp."")->result();
			...
			foreach($hasil as $row):
				...
				#Cek Cek Status Unit
				// if (substr($row->unit_no,2,2) == 02){
					// $bgcolor ='#9400D3';
				// }elseif(substr($row->unit_no,2,2) == 03){
					// $bgcolor ='#FF1493';
				// }elseif(substr($row->unit_no,2,2) == 05){
					// $bgcolor ='#00BFFF';
				// }else
				...
				if($no==$max){
					echo "<td bgcolor=".$bgcolor." style='height:24px'><a class='thickbox' href='form_alokasi_sardjito/".$row->unit_id."?width=800&height=500' title='Reserved Unit'>".$row->unit_no."</a></td>";
					echo"<tr>";
					$no = 0;
				}else{
					
					echo "<td bgcolor=".$bgcolor."><a class='thickbox' href='form_alokasi_sardjito/".$row->unit_id."?width=800&height=500' title='Reserved Unit'>".$row->unit_no."</a></td>";
		//	
...
<!--	<table class="wrapheader" border="0">	
		...
		<tr>
			...
			<td><b>| Nett. <?=$data['reserved']->land?> Sqm </b></td>
			<td><b>| SGA. <?=$data['reserved']->bgnan?> Sqm </b></td>
		</tr>
		<tr>
			<td class="class3" colspan="3">Sold</td>
			<td><b><?=$data['sold']->status?> ,Unit</b></td>
			<td><b>| Nett. <?=$data['sold']->land?> Sqm </b></td>
			<td><b>| SGA. <?=$data['sold']->bgnan?> Sqm </b></td>
		</tr>
		<tr>
			<td class="class4" colspan="3">N/A</td>
			<td><b><?=$data['na']->status?> ,Unit</b></td>
			<td><b>| Nett. <?=$data['na']->land?> Sqm </b></td>
			<td><b>| SGA. <?=$data['na']->bgnan?> Sqm </b></td>
		</tr>
		<tr>
			<td class="class5" colspan="3">Proses Dealing</td>
			<td><b><?=$data['pd']->status?> ,Unit</b></td>
			<td><b>| Nett. <?=$data['pd']->land?> Sqm </b></td>
			<td><b>| SGA. <?=$data['pd']->bgnan?> Sqm </b></td>
		</tr>
...
<table id="tabel" border="0">	
		...
		<tr>
			<td class="class5" colspan="3">Blok Unit</td>
