...
if($poin){
  ...
  return $hasil; 
			...
			$pdf=new PDF('P','mm','A4');
						if($pt==44){
						$data = $this->db->query("select * from db_apinvoiceoth where doc_no='".$doc_no->doc_no."'")->result();
						}elseif(($pt==11) ){
						...
						//var_dump($data);exit;
							 	...
							 	$rows = $this->db->query("sp_cetakpayvoucherallnew '".$id."'")
							 ->row();
			//var_dump($data);
			...
			$pdf->Cell(2,5,':',10,0,'L');
			$pdf->SetFont('Arial','',9);
			$pdf->Cell(30,4,indo_date($rows->doc_date),10,0,'L');
			$pdf->Ln(5);
			...
			$pdf->Cell(4,5,':',10,0,'L');
			$pdf->Cell(55,4,number_format($rows->trx_amt),10,0,'L');
			$pdf->Ln(5);
			...
			$pdf->Write(5,ucwords(terbilang($rows->trx_amt)));
			$pdf->Ln(8);
