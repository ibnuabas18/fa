		...
		$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						border:false,
						cache:true,
						href:"<?=site_url('bankpayment/show_form')?>/"+xref+"/?index="+index,
						onLoad:function(){
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this Record?',function(r){
					if (r){
						...
						$.post('<?=site_url('bankpayment/delete')?>',{id:row.id},function(){
							$('#dg').datagrid('deleteRow',index);
						});
					}
				});
...
<form id="formAdd" method="post" action="<?=site_url('cb/bankpayment/saveheader')?>">
 <?php $tgl = date('d-m-Y'); ?>
<body>
...
<div class="easyui-tabs" style="width:1100px;height:300px;">
	<div title="Add Jurnal Bank" style="padding:10px;">	
		<table id="dg" title="Jurnal Bank" style="width:1100px;height:250px"
				url="<?=site_url('bankpayment/get_dg')?>"
