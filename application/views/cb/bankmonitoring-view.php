	 		...
	 		$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						...
						href:"<?=site_url('bankmonitoring/show_form')?>/"+des+"/?index="+index,
...
<form id="formAdd" method="post" action="<?=site_url('bankmonitoring/saveheader/'.$data->id_cash.'')?>">
<body>
		...
		<td>Voucher</td>
			...
			<td>
				...
				<select  name="bank" id="bank" >
					<option value="<?=@$data->bank_id?>"><?=@$bank->bank_id?></option>
					<?foreach($bank as $row):?>
	...
	<tr>	
			...
			<td>Amount</td>			
			<td>:</td>
			<td><input type="text" class="calculate input validate[required]" name="amount" id="amount"  value="<?=(@$data->base_amount)?>"   size="30" /></td>
	</tr>
	<tr>
	<td>Terima Dari</td>
			<td>:</td>
