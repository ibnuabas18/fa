	...
	return $('#mytable').jqGrid(gridData)
		...
		.navButtonAdd('#pager',{
		   ...
		   onClickButton: function(){ 
		    var id = getSelectedID();
			 if(id){
				 popupForm('<?php echo site_url();?>cb/bankkeluar/payment/' + id + '/?width='+1200+'&height='+400);	
			 }else{
		...
		.navButtonAdd('#pager',{
		   ...
		   onClickButton: function(){ 
			 var id = getSelectedID();
			 if(id){
				 window.open('<?php echo site_url();?>cb/bankkeluar/print_slip/' + id + '/?width='+900+'&height='+500);
			 }else{
		...
		.navButtonAdd('#pager',{
		   ...
		   onClickButton: function(){ 
			 var id = getSelectedID();
			 if(id){
				 popupForm("<?=site_url()?>" + 'cb/bankkeluar/pyd/' + id + '/?width='+900+'&height='+250);
			 }else{
		...
		.navButtonAdd('#pager',{
		   ...
		   onClickButton: function(){ 
			 var id = getSelectedID();
			 if(id){
				 popupForm("<?=site_url()?>" + 'cb/bankkeluar/paid/' + id + '/?width='+900+'&height='+250);	
			 }else{
				 alert('Pilih baris yang ingin di View');
			 }
		   }, 
...
/*function cellColumn(cellVal,opts,element){
	...
	//alert(id);
	$.getJSON('<?php echo site_url();?>cb/bankkeluar/getidcash/'+id,function s(get){
	return get.status;
...
<table >
		<tr>
			<td style="background-color:pink;width:130px;height:25px" >Cek Gantung</td>
			<td style="background-color:#FF6600;;width:130px;height:25px">Terbayar</td>
