	...
	//$(function(){
			...
			$(".calculate").bind('keyup keypress',function(){
				...
				$('#scost').change(function(){
					...
					var idp = $('#proj_id').val(); 
					var stru = $('#scost').val(); 
					$.getJSON('<?=site_url('proposedbgt/showdes')?>/'+idp+'/'+stru,
							function(data){
							  if(data.error == undefined){ // jika respon error tidak terdefinisi maka pengambilan data sukses 
								 $('#codebgt').empty(); // kosongkan dahulu combobox yang ingin diisi datanya
								 $('#codebgt').append('<option></option>'); // buat pilihan awal pada combobox
								 for(var x=0;x<data.length;x++){
									// berikut adalah cara singkat untuk menambahkan element option pada tag <select>
									$('#codebgt').append($('<option></option>').val(data[x].id).text(data[x].nama));
								 }
								 $('#loading').text(''); // hilangkan text loading
							  }else{
								 alert(data.error); // jika ada respon error tampilkan alert
					...
					//loadData('codebgt',$('#scost option:selected').val());
				});			
				...
				// $('#sscost').change(function(){
					...
					// var proj = $('#proj_id option:selected').val();
					// var sscost = $('#sscost option:selected').val();
					// $.post('<?=site_url('proposedbgt/showbgt')?>',
						// {'proj':proj,'sscost':sscost},
						// function(data){
							// $('#codebgt').empty(); 
							// $('#codebgt').append('<option></option>'); // buat pilihan awal pada combobox
							 // for(var x=0;x<data.length;x++){
...
<form method="post">
	<table class="dv-table" style="width:100px;background:#fafafa;padding:5px;margin-top:5px;">
		<input type="hidden" value="<?=$no_bgt?>" name="no_bgt" id="no_bgt" value = "1"/>		
		<input  type='hidden'name="prj" id="prj" value="2"/>		
		<tr>
<!--
			<input style="width:180px" name="jobdet" class="easyui-validatebox" required="true" >
-->
			<select style="width:170px" name="proj_id" id="proj_id"></select>
