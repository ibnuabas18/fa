<?php
			...
			$pdf->Cell(18,5,'Previous',1,0,'C',1);
			$pdf->Cell(18,5,'Deduct',1,0,'C',1);
			$pdf->Cell(18,5,'Current',1,0,'C',1);
			...
			#die($tanggal);
			$query = $this->db->query("sp_rptcontract '".$tanggal."'")->result();
			$totclaim = 0;
			$totbalance = 0;
			$totkontrak = 0;
			$deduct = 0;
			...
			foreach($query as $row){
				...
				if($row->currency == 'USD'){
						$kontrak 	=	$row->contract_amount * 9700;
						
						$current 	= $kontrak - $row->deduction;
						...
						$deduct = $deduct + $row->deduction;
						$totcurrent = $totcurrent + $current;
				}
				else{
						$current 	= $row->contract_amount - $row->deduction;
			...
			$pdf->Cell(45,5,'Kontrak Kerja',1,0,'C',1);
			$pdf->Cell(40,10,'Contractor',1,0,'C',1);
			$pdf->Cell(100,10,'J O B',1,0,'C',1);
			$pdf->SetFont('Arial','',7);
			#$pdf->Cell(19,6,'Amount (IDR)',1,0,'C',1);
			$pdf->Cell(19,6,'Total Contract',1,0,'C',1);
			$pdf->SetFont('Arial','',8);
			$pdf->Cell(22,10,'Claim',1,0,'C',1);
			#$pdf->Cell(22,10,'Add/Deduct',1,0,'C',1);
			$pdf->Cell(22,10,'CTC',1,0,'C',1);
			...
			#$pdf->Cell(60,0,'',0,0,'R',0);
			$pdf->SetX(12);
			$pdf->Cell(25,5,'Number',1,0,'C',1);
			$pdf->Cell(20,5,'Date',1,0,'C',1);
			
			$pdf->SetX(160);
			$pdf->SetFont('Arial','',7);
			$pdf->Cell(19,5,'(Excl.Tax)','L',0,'C',1);
			$pdf->Cell(18.7,5,'(Incl.Tax)','L',0,'C',1);
				...
				$pdf->SetFont('Arial','',6);
				$pdf->setFillColor(222,222,222);
				$pdf->Cell(165,6,'GRAND TOTAL',1,0,'R',1);
