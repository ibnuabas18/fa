<?php
		...
		$pdf->Image(site_url().'assets/images/bakrie_gmi.JPG',4,8,20);	
			...
			$pdf->SetX(5);
			$pdf->Cell(50,5,'Certified Number.',10,0,'L');
			$pdf->Cell(2,5,':',10,0,'L');
			$pdf->Cell(30,5,@$rows->no_cjc,10,0,'L');
			$pdf->Ln(8);
			
			$pdf->SetX(5);
			$pdf->Cell(50,5,'Contract Number.',10,0,'L');
			$pdf->Cell(2,5,':',10,0,'L');
			...
			$pdf->Cell(2,5,':',10,0,'L');
					#$prevprog = @$progress->totprog - @$progress->prog;
			$pdf->Cell(40,5,number_format(@$progress->prev).' %',10,0,'R');
			$pdf->Ln(8);
			
			$pdf->SetX(5);	
			$pdf->Cell(50,5,'This Term Progress (%)',10,0,'L');
			$pdf->Cell(2,5,':',10,0,'L');
			$pdf->Cell(40,5,number_format(@$progress->prog).' %',10,0,'R');
			$pdf->Ln(8);
			
			$pdf->SetX(5);	
			$pdf->Cell(50,5,'Previous Value(Rp)',10,0,'L');
			$pdf->Cell(2,5,':',10,0,'L');
					
			$pdf->Cell(40,5,number_format(@$value->prevvalue),10,0,'R');
			$pdf->Ln(8);
			
			$pdf->SetX(5);	
			
			$pdf->Cell(50,5,'This Term Value(Rp) ',10,0,'L');
			$pdf->Cell(2,5,':',10,0,'L');
			...
			$pdf->Cell(40,5,'('.ucwords(toRupiah(@$rows->claim_amount)).' Rupiah)',10,'b','L');
			
			$pdf->Ln(8);
			$pdf->SetX(5);	
			$pdf->SetFont('Arial','','10');
			$pdf->Cell(50,5,'Balanced Value(Rp) ',10,0,'L');
			$pdf->Cell(2,5,':',10,0,'L');
			$pdf->SetFont('Arial','','10');
					$balanced = @$value->totkontrak - (@$rows->claim_amount + @$value->prevvalue);
			$pdf->Cell(40,5,number_format($balanced),10,0,'R');
			#$pdf->Cell(40,5,'(Incl. PPN 10%)',10,0,'L');
			$pdf->Ln(8);
			...
			$pdf->Cell(40,6,substr(@$rows->nm_supplier,0,23),'L'.'R'.'T',0,'C',0);
			$pdf->SetFont('Arial','',9);
			...
			$pdf->Cell(40,6,substr(@$rows->nm_supplier,23,50),'L'.'R',0,'C',0);
