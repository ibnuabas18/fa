		...
		$("#add").click(function(){
			...
			if(stat_add=='Add'){
				...
				$("#cancel2").show();
					...
					//alert(data);
					$.getJSON('<?=site_url()?>tendereval/tambah/'+vendor+'/'+offe+'/'+score+'/'+remark + '/' + idtr + '/' + nego, function(data) {
					var row="<tr id='"+data.id_vendor+"'><td>"+data.vendor_nm+"</td><td>"+data.offer_ven+"</td><td>"+data.nego_ven+"</td><td>"+data.score_ven+"</td><td>"+data.remark_ven+"</td></tr>";
				//alert(row);	
				$("table#vendor tbody").append(row);
				
				//Input On Combobox				
				//$('#tendwin').append('<option></option>');
				$('#tendwin').append($('<option></option>').val(data.participant_id).text(data.vendor_nm));
				$("#cancel2").hide();
				btn_add.empty();
				btn_add.text('Add');
				})	
				...
				function(data){
					if(data.error == undefined){
						...
						for(var x=0;x<data.length;x++){
							...
							$('#tendwin').append($('<option></option>').val(data[x].participant_id).text(data[x].vendor_nm));
							//alert(row);
	...
	<table width="800px">
		...
		<tbody>
			<tr>
				...
				<td width="150px"><input type="text" name="offe" id="offe" class="chinput input calculate rounded-corners validate[required]" maxlength="20" readonly="true"/></td>
				<td width="150px"><input type="text" name="nego" id="nego" class="chinput input calculate rounded-corners validate[required]" maxlength="20" readonly="true"/></td>
				<td width="150px"><input type="text" name="score" id="score" class="chinput input rounded-corners validate[required]" readonly="true"/></td>
				<td width="150px"><input type="text" name="remark" id="remark" class="chinput rounded-corners validate[required]" readonly="true"/></td>
			</tr>
		</tbody>
	</table>
