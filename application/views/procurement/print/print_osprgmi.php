<?php
			...
			$data = $this->db->query("sp_osprgmi '".$div."','".$kat."','".$ven."','".inggris_date($start_date)."','".inggris_date($end_date)."'")
							 ->result();
	...
	foreach($data as $row){
		...
		//
			...
			$pdf->Cell(25,5,substr($row->no_pr,10,15),1,0,'L',0);
			$pdf->Cell(50,5,$row->nm_supp,1,0,'L',0);
			$pdf->Cell(95,5,substr($row->ket_pr,0,78),1,0,'L',0);
			//$pdf->Cell(95,5,$row->ket_pr,1,0,'L',0);
			$pdf->Cell(25,5,number_format($row->harga_tot),1,0,'R',0);
			$pdf->Cell(35,5,$row->divisi_nm,1,0,'C',0);
