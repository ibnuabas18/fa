<?php
	//die($id);
	$rows = $this->db->query("sp_printmrr '".$id."'")->row();
	...
	$i = 1;	
			...
			$max = 45;	
			$tot = 0;
	$dt = $this->db->select('qtyPO,qtyMsk,a.satuan satuan,a.nm_brg nm_brg,no_mrr')
					->join('db_barangpod b','b.brgpod_id = a.brgpod_id','left')
				   ->join('db_barangpoh c ','c.brgpoh_id = a.brgpoh_id','left')
				   ->join('db_divisi d','d.divisi_id = c.divisi_id','left')
				   ->join('pemasokmaster e','e.kd_supplier = c.kd_supp','left')
				   ->where('brgmsk_id',$id)
				   ->get('db_barangpomsk a')->result();
