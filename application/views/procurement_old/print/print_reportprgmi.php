<?php
			...
			$data = $this->db->query("sp_allreportprgmi '".$div."','".$kat."','".$ven."','".inggris_date($start_date)."','".inggris_date($end_date)."'")
							 ->result();
			//var_dump($ven);
			$pdf=new PDF('L','mm','A4');
				if ($kat == 1){
			...
			$pdf->Cell(33,5,'PR',1,0,'C',1);
			$pdf->Cell(42,5,'PO',1,0,'C',1);
			...
			$pdf->Cell(24,5,'Lead Time',1,0,'C',1);
			$pdf->Cell(15,10,'Remark',1,0,'C',1);
			...
			$pdf->Cell(20,5,'',0,0,'R',0);
			$pdf->Cell(12,5,'PR to PO',1,0,'C',1);
			$pdf->Cell(12,5,'PO to MRR',1,0,'C',1);
		...
		$pdf->Ln(25);
			...
			$no = 0;
				...
				$pdf->Cell(14,5,indo_date($row->tgl_aproval),1,0,'L',0);
				$pdf->Cell(19,5,substr($row->no_pr,10,15),1,0,'L',0);
				$pdf->Cell(14,5,$tglpo,1,0,'L',0);
				$pdf->Cell(28,5,$row->no_po,1,0,'L',0);
				$pdf->Cell(14,5,$tglmrr,1,0,'L',0);
				$pdf->Cell(23,5,$row->no_mrr,1,0,'L',0);
				$pdf->Cell(43,5,substr($row->nm_supp,0,39),1,0,'L',0);
				$pdf->Cell(43,5,substr($row->ket_pr,0,30),1,0,'L',0);
				$pdf->Cell(20,5,number_format($row->harga_tot),1,0,'R',0);
				$pdf->Cell(20,5,$row->divisi_nm,1,0,'L',0);
				$pdf->Cell(12,5,$row->d1,1,0,'C',0);
				$pdf->Cell(12,5,$row->d2,1,0,'C',0);
				$pdf->Cell(15,5,$app,1,0,'L',0);
				$pdf->Ln(5);				
				$i++;
				$no++;
			...
			//	$totamount = number_format($totamount);
				...
				$pdf->Cell(47,5,'',0,0,'L',0);
				$pdf->Cell(43,5,'TOTAL',1,0,'L',1);
				$pdf->Cell(15,5,number_format($tothrg),1,0,'R',1);
