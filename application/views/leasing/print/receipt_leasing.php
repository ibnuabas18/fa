<?php
	...
	$nopem = $cekdt->kwitansi_no;
	$tglreceipt = $cekdt->kwitansi_paydate;
	$bank = 'BTN';
	$acctno = '-';
	...
	$pdf->Ln(5);
	$pdf->Cell(8,5,'Pembayaran '.$cekdt->paygroup_nm.' '.$cekdt->customer_nama.' Unit '.$cekdt->unit_no.'--'.$cekdt->kwitansi_descs,10,0,'L',0);
	$pdf->Cell(164,5,'',10,0,'L',0);
	$pdf->Ln(5);
	$pdf->Cell(8,5,'',10,0,'L',0);
	#akhir konsisi data pembayaran
			
	$pdf->Cell(155,5,'Terbilang '.Ucfirst(toRupiah($cekdt->kwitansi_pay)),10,0,'L',0);
	$pdf->Cell(20,5,number_format($cekdt->kwitansi_pay),10,0,'L',0);
	$pdf->Ln(10);
	#template tabel
