...
$(function(){
   ...
   $('#proj').change(function(){
	   if($('#proj option:selected').val() != '')
			loadData('unit_leasing',$('#proj option:selected').val());
   });
   ...
   $('input:checkbox').change(function(){
		if($("input:checkbox:checked").val()==1) {
			//alert("true");
			$('#unit_leasing').empty();
			$('#unit_leasing').append($('<option></option>').val('All').text('All'));
		}else{
			loadData('unit_leasing',$('#proj option:selected').val());
		}
   });   
...
<h2><font color='red' size='4'>Historical Payment Report<hr width="150px" align="left"></font></h2>
<div class="printed">
<form method="post" action="<?=base_url()?>sales/report_sales/history_pembayaran_report_leasing" target="_blank">
<table border="0">
	<tr>
		<td>Project</td>
		<td>:</td>
	...
	<tr>
		...
		<td>:</td>
		<td>
			<select name="unit_leasing" id="unit_leasing"></select>
		</td>
		<!--<td><input type="checkbox" name='all' id='all' value='1' /></td>-->
		<td><input type="hidden" name="checkbox" id="checkbox" value="0" /> <input type="checkbox" id="checkbox" name="checkbox" value="1" /></td>
		<td>All</td>
	</tr>
	<tr>
		<? $tgl = date("d-m-Y");?>
