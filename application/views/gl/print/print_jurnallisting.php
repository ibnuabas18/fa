<?php
			...
			// $sql = 	"SELECT customer_id,unit_no,customer_nama,customer_alamat2,customer_hp,nama FROM db_sp a
					// JOIN db_customer b ON a.id_customer = b.customer_id
					// JOIN db_unit_yogya ON id_unit = unit_id
					// JOIN db_kary ON a.id_sales=id_kary
			...
			// }else{
				// $sql = 	"SELECT customer_id,unit_no,customer_nama,customer_alamat2,customer_hp,nama FROM db_sp a
					// JOIN db_customer b ON a.id_customer = b.customer_id
					// JOIN db_unit_bdm ON id_unit = unit_id
					// JOIN db_kary ON a.id_sales=id_kary
					// WHERE ISNULL(a.id_flag,0) <> 10 AND a.id_subproject = $proj
					// ORDER BY customer_nama";
			// $rw = $this->db->query($sql)->result();
			...
			$rw = $this->db->query("CustomerAddressReport '".$proj."','".$pt."'")
							 ->result();


			$nm_proj = $this->db->select('nm_subproject')
										->where('subproject_id',$proj)
										->get('db_subproject')
										->row();
										
			$nm_pt = $this->db->select('ket')
										->where('id_pt',$pt)
										->get('pt')
										->row();
			...
			#Header
				...
				$pdf->SetX(25);
				$pdf->Cell(0,10,$nm_pt->ket,20,0,'L');
				...
				if($proj == 41011){
					...
					$proje = 'HADININGRAT TERRACE';
			}
				$pdf->SetFont('Arial','',9);
				$pdf->Cell(0,10,'PROJECT : '.$nm_proj->nm_subproject,20,0,'L');
				$pdf->SetXY(25,26);
	...
	$a = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa4";
		//~ $tgl_lhr = indo_date($row->customer_tgl_lhr);
			...
			$pdf->Cell(20,10,$row->unit_no,1,0,'C',0);
			//~ $pdf->Cell(20,5,indo_date($row->tgl_sales),1,0,'C',0);
			$pdf->Cell(50,10,$row->customer_nama,1,0,'L',0);
			//~ $pdf->Cell(20,5,$tgl_lhr,1,0,'C',0);
			//~ $pdf->Cell(15,5,$row->customer_tlp,1,0,'L',0);
			//$pdf->Cell(50,10,substr($row->descs,0,40),"L"."R",0,'L',0);
			$pdf->Cell(125,10,substr($row->customer_alamat2,0,110),"L"."R",0,'L',0);
			//~ $pdf->Cell(15,5,$row->kdpos1,1,0,'C',0);
			$pdf->Cell(20,10,$row->customer_hp,1,0,'L',0);
			//~ $pdf->Cell(15,5,$row->customer_fax,1,0,'L',0);
			//~ $pdf->Cell(10,5,$row->umur,1,0,'C',0);
			$pdf->Cell(35,10,$row->nama,1,0,'L',0);
		//~ 
			$pdf->Ln(5);

			$pdf->SetX(84);
					
			$pdf->Cell(125,5,substr($row->customer_alamat2,110,110),'L'.'R'.'B',0,'L',0);
