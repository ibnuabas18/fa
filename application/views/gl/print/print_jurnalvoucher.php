<?php
			...
			$pdf=new PDF('P','mm','A4');
						...
						$data = $this->db->query("sp_cetakjurnalvoucher2 '".$voucher."'")
							 ->result();
							 
							 	$rows = $this->db->query("sp_cetakjurnalvoucher '".$voucher."'")
							 ->row();
			//var_dump($data);
			...
			$pdf->Cell(2,5,':',10,0,'L');
			$pdf->Cell(30,3.5,$rows->doc_date,1,0,'L');
			$pdf->Ln(3);
			$pdf->SetFont('Arial','',6);
			...
			$pdf->SetX(25);
			$pdf->Cell(110,5,'JURNAL MEMORIAL',10,0,'L');
			//$pdf->SetFont('Arial','',6);
