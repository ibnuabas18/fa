<?php
			...
			// $data = $this->db->query("sp_printgeneralledger '".inggris_date($tgl)."'")
							 // ->result();
									 ...
									 $rows= $this->db->query("sp_printdebtoroutstanding '".inggris_date($tgl)."', '".inggris_date($tgl2)."'")
							 ->result();
			...
			#HEADER CONTENT
				...
				$judul 		= "DEBTOR OUTSTANDING DETAILS REPORT";
				$periode	= "Periode";
			...
			$pdf->Cell(15,10,'',10,0,'C',0);
			$pdf->Cell(20,10,'Unit No',10,0,'L',0);
			$pdf->Cell(30,10,'Customer Name',10,0,'C',0);
			$pdf->Cell(78,10,'Description',10,0,'L',0);
			$pdf->Cell(15,10,'Due Date',10,0,'C',0);
			$pdf->Cell(15,10,'Days',10,0,'C',0);
			$pdf->Cell(30,10,'Amount',10,0,'R',0);
			$pdf->Ln(5);
...
//for($h = 1;$h <= 2; $i++){		
	foreach($rows as $row){	
			...
			$data2 = $this->db->query("sp_printdebtoroutstandingdetail '".$voucher."'")
							 ->result();
...
$pdf->SetFont('Arial','',7);
					...
					$b = $b + $roow->credit;
							...
							$pdf->Cell(78,5,$roow->acc_no,10,0,'L',0);
							$pdf->Cell(15,5,indo_date($roow->acc_name),10,0,'L',0);
							$pdf->Cell(15,5,($roow->debit),10,0,'C',0);
...
//	}
			...
			$pdf->Cell(20,5,'',10,0,'L',0);
			$pdf->Cell(30,5,'',10,0,'R',0);
			$pdf->Cell(78,5,'Grand Total :',10,0,'R',0);
			$pdf->Cell(15,5,'',10,0,'L',0);
			$pdf->Cell(15,5,'',10,0,'C',0);
			$pdf->Cell(30,5,number_format($b),10,0,'R',0);
			$pdf->Ln(3);		
