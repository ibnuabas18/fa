...
function saveItem(index){
			...
			var url = row.isNewRecord ? '<?=site_url('viewjurnal/savedetail')?>' : '<?=site_url('viewjurnal/savedetail')?>/'+row.id;
			...
			$.getJSON('<?=site_url()?>viewjurnal/lempar/'+ voucher,
			function(lempar){
		...
		if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this Record?',function(r){
					if (r){
						...
						$.post('<?=site_url('viewjurnal/delete')?>/'+ a,
						//{
						//id:row.id},
						function(){
							$('#dg').datagrid('deleteRow',index);
	 		...
	 		$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						border:false,
						cache:true,
						//href:"<?=site_url('viewjurnal/show_form2')?>/"+xref+"/"+rem+"/?index="+index,
						href:"<?=site_url('viewjurnal/show_form2')?>/"+xref+"/"+"/?index="+index,
						onLoad:function(){
...
<form id="formAdd" method="post" action='<?=site_url('viewjurnal/to_approve')?>'>
 <?php $tgl = date('m/d/Y'); ?>
<body>
<input type="hidden" name="gl_id" value="<?=$data->gl_id?>">
<table>
	<h2>Journal Transfer</h2>		
	</div>
	<!--<tr>
	<td>Trx Type</td>
			<td>:</td>
	...
	<tr>
		<td>Description</td>
			<td>:</td>
			<td><input type="text" name="remark" id="remark"  value="<?=@$data->desc?>" class="xinput validate[required]" style="background-color:#EFFC94" readonly="true" size="80" /></td>
	</tr>
	<tr>
	<td>
	</td>	
...
<div class="easyui-tabs" style="width:1000px;height:250px;">
	<div title="Add Detail Journal" style="padding:10px;">	
		<table id="dg" title="View Journal" style="width:980px;height:200px"
				url="<?=site_url('viewjurnal/get_dg')?>/<?=$data->gl_id?>"
	...
	<tr>
	<input type="submit" name="save" value="Posting"/>
