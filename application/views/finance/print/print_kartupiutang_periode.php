...
//class print_kartu_piutang_lease extends controller{
	//function kartu_piutang($id){
			...
			//$id=144;
				//var_dump($id);
				$query = $this->db->query("ViewCustomerleaseperiode " .$unit."");
			$data	= $query->result();	
			...
			$pdf->SetFont('Arial','',10);		
				...
				$sumtotalbilling2 = $this->db->select('sum(trx_amount) as bill_amount')
							->where('no_kontrak',$row->no_kontrak) 
							->where_not_in('id_flag',10)
							->get('db_invoice')->row();
				...
				// $pdf->Cell(30,4,number_format($sumtotalbilling2->bill_amount),0,0,'R',0);
				...
				// #TOTAL BALANCED
				// $totbalbill = $sumtotalbilling2->bill_amount - $sumtotalbilling->pay_amount;
				...
				$pdf->Cell(30,4,$head_nama,0,0,'L',0);
					...
					$pdf->Cell(8,4,':',4,0,'L');
						...
						#HEAD TABLE
							...
							$pdf->Cell(20,7,'Invoice Date',1,0,'C',0);
							$pdf->Cell(18,7,'Pay Date',1,0,'C',0);
							$pdf->Cell(30,7,'Pay No',1,0,'C',0);
							$pdf->Cell(25,7,'Due Amount',1,0,'C',0);
							$pdf->Cell(25,7,'Pay Amount',1,0,'C',0);
...
//					$pdf->SetXY(30,82);
			...
			$query2 = $this->db->query("ViewCustomerleasedetailperiode " .$unit.",'" .inggris_date($tgl1)."','" .inggris_date($tgl2)."'");
			$cek	= $query2->result();	
	...
	foreach($cek as $row){
	#DUE DATE
	$duedate = indo_date($row->tgl_invoice);
	
	#TOTAL BILLING
	$totbill = $totbill + ($row->trx_amount);
	...
	$no++;			
			...
			// $dt = $this->db->select('count(id_paygroup) as id')
				...
				$os = ($row->trx_amount+$row->stamp) - $row->payment;
				...
				$pdf->Cell(20,6,$duedate,1,0,'C',0);
				$pdf->Cell(18,6,$row->tgl_payment,1,0,'C',0);
				$pdf->Cell(30,6,'',1,0,'C',0);
				$pdf->Cell(25,6,number_format($row->trx_amount+$row->stamp),1,0,'R',0);
				$pdf->Cell(25,6,number_format($row->payment),1,0,'R',0);
				...
				$pdf->SetFont('Arial','B',10);		
				$pdf->Cell(106,4,'Total',1,0,'R',0);
				$pdf->Cell(25,4,number_format($totbill),1,0,'R',0);
				$pdf->Cell(25,4,number_format($subpay),1,0,'R',0);
