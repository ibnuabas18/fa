<?php
	//die($idnama);
			...
			$pdf->SetFont('Arial','',10);
			$pdf->Ln(12);
			$pdf->Cell(35,0,'Perihal : Pemberitahuan Kedua Pembayaran Pemesanan  '.$rows->project_no.' : '.$rows->unit_no.'',10,0,'L');
			//$pdf->SetFont('Arial','',10);
			//$pdf->Cell(15,0,'Pemberitahuan Kedua Pembayaran Pemesanan  '.$rows->project_no.' : '.$rows->unit_no.'',10,0,'L');
			$pdf->SetFont('Arial','',10);
			...
			$pdf->Ln(4);
							...
							$pdf->Ln(4);
							$pdf->Cell(40,0,'Menindaklanjuti surat kami terdahulu tertanggal  '.indo_date($rows->last_date).', no  '.$rows->ref_no.'' ,10,0,'L');	
							$pdf->Ln(4);
							$pdf->Cell(40,0,' Perihal  Pemberitahuan  Pertama  Pembayaran  Pemesanan  Unit  '.$rows->project_no.',',10,0,'L');	
			
							#end header
							$pdf->Ln(4);
							$pdf->Cell(40,0,'kami  mohon  perhatian  Bapak/Ibu  sebagai  berikut  :',10,0,'L');	
		...
		$pdf->Ln(2);
			...
			$pdf->Ln(4);
			$pdf->Cell(55,6,'Apabila  dalam   waktu   tersebut   belum  juga  memenuhi  kewajiban  tersebut, maka kami akan melakukan tindakan ',10,0,'L',0);
			$pdf->Ln(4);
			$pdf->Cell(55,6,'sesuai     syarat    dan    ketentuan    Surat   Pesanan   dan /  atau   Perjanjian    Pengikatan   Jual  Beli    yang    telah  ',10,0,'L',0);
			$pdf->Ln(4);
			$pdf->Cell(55,6,'ditandatangani  oleh PT. '.$rows->project.'  dan  Bapak/Ibu.',10,0,'L',0);
			$pdf->Ln(4);
