...
<form method="post" action="<?=site_url('print/slip_budgetedit')?>"  id="formAdd" target='_blank'>
<table border="0"  cellpadding="0" cellspacing="0">
	...
	<input type="hidden" name="blcalldiv" class="input" id="blcalldiv" readonly="true"/>
		...
		<td>Code</td>
			...
			<td colspan='8'>BUDGET : <select  name="code" id="code" >
					<option><?=@$code?></option>
					<?foreach($bank as $row):?>
						<option value="<?=$row->id?>"><?=$row->nama?></option>
					<?endforeach?>
...
-->
		...
		<tr>
			...
			<? if ($pt == '44'){ ?>
				<td colspan='6'>
				<input type="radio" name="flag_pr" id="flag_pr" value="1" class="validate[required] radio"> Budget PR
		...
		<tr>
				...
				<td>:</td> 
				<td colspan='7'><input type="text" name="amount" id="amount" class="xinput calculate input validate[required]" value="<?=@$data->amount?>"/></td>
		</tr>
		...
		<tr>
				...
				<td>Remark</td> 
				<td>:</td> 
				<td><input "text " name="remark" id="remark" class="validate[required] xinput" xinput" value="<?=@$data->remark?>"  style="width:300px;height:20px" /></td>		
			
		</tr>
		
		<tr>
			<td colspan ='9'>
				<input type="text" name="noid" id="noid" value="<?=@$data->id_trbgt?>"/>
