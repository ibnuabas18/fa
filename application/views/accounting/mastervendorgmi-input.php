...
$(function(){
		 ...
		 $('select[name=kd_supplier1]').change(function(){
			 ...
			 $.getJSON('<?=site_url('mastervendorgmi/data3')?>/'+$(this).val(),
				function(data3){
		 ...
		 $('select[name=kelusaha]').change(function(){
			$.getJSON('<?=site_url('mastervendorgmi/data4')?>/'+$(this).val(),
				function(data4){
...
<script language="javascript">
   function loadData(type,parentId){
      ...
      $.post('<?=site_url('mastervendorgmi/loaddata')?>', //request ke fungsi load data di inputAP
...
<body>
<form action="<?=site_url()?>mastervendorgmi/save" method="post"  id="formAdd">
<table>
	<tr>
	...
	<tr id="trnama">
		...
		<td>:</td>
		<td>
			<input type="text" disabled="disabled" onfocus="true"  class="validate[required]" name="nm_supplier" id="nm_supplier"  value="<?=@$data->nm_supplier?>"  size="21"   />
	...
	<tr id="trkategori">
		...
		<td>
			...
			<input type="hidden" readonly="true" name="kel_usaha" id="kel_usaha"  size="21" />
		</td>	
	</tr>
	<tr id="trnpwp">
		<td>NPWP</td>
		<td>:</td>
		<td><input type="text" name="npwp"  class="validate[required]" id="npwp" value="<?=@$data->npwp?>" required  size="21" /></td>
	</tr>
	<tr id="trakte">
		<td>Akte</td>
		<td>:</td>
