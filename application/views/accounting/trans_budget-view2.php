...
$id = str_replace("0","",$m);
$jbln = $arr[$id];
$tot3 = $this->mstmodel->total_month($kode,$jbln,$Y,$pt);
...
#Budget Annual
$tot1 = $this->mstmodel->total_annual($kode,$Y,$pt);
...
for($i=1;$i<=$m;$i++){
	$n = $arr[$i];
	$tot5 = $this->mstmodel->total_ytd($n,$kode,$Y,$pt);
	$hsl = $tot5['jml'];
	$totytd = $totytd + $hsl;
}	
...
for($i=1;$i<=$m;$i++){
	$n = $arr[$i];
	$tot5div = $this->mstmodel->total_ytddiv($n,$divisi,$Y,$pt);
	$hsldiv = $tot5div['jml'];
...
$dtactdiv = $this->mstmodel->appact_annualdiv($divisi,$Y);
$totdiv = $dtdiv->jml;
...
$namabudget = $this->mstmodel->get_namabudget($data->id_trbgt,$Y);
$nama		= $namabudget['descbgt'];
$code		= $namabudget['code'];

#status
$status	= $this->mstmodel->trbgtdiv($data->id_trbgt);
$status_budget = $status->status_bgt;
...
<form method="post" action="<?=site_url('trans_budget/save_edit')?>"  id="formAdd" target="_blank">
	...
	<table border="0" width="850px">
		...
		<tr>
				...
				<td>:</td> 
				<td><span><?=number_format($data->appamount)?></span></td>
		...
		<tr>
			...
			<tr>
				...
				<td>Proposed Amount </td>
				<td>:</td> 
				<td colspan='7'><input type="text" name="amountedit" id="amountedit" class="xinput calculate input validate[required]" value="<?=$amount?>"/></td>
			</tr>
			<tr>
				<td>Remark</td> 
				<td>:</td>
				<td><input "text " name="remarkedit" id="remarkedit" class="validate[required] xinput" xinput" value="<?=@$data->remark?>"  style="width:300px;height:20px" /></td>		
			</tr>
			<tr>
			<td colspan ='9'>
				<input type="hidden" name="noid" id="noid" value="<?=@$data->id_trbgt?>"/>
				<input type="submit" id="klik" name="proposed" value="Save"/>
