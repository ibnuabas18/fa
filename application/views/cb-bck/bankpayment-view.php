...
// function loadData(type,parentId){
   ...
   function saveItem(index){
			...
			var url = row.isNewRecord ? '<?=site_url('bankpayment/savedetail')?>' : '<?=site_url('bankpayment/savedetail')?>/'+row.id;
			...
			$.getJSON('<?=site_url()?>bankpayment/lempar/'+ voucher,
			function(lempar){
		...
		if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this Record?',function(r){
					if (r){
						...
						$.post('<?=site_url('bankpayment/delete')?>/'+ a,
						//{
						//id:row.id},
						function(){
							$('#dg').datagrid('deleteRow',index);
	 		...
	 		$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						border:false,
						cache:true,
						href:"<?=site_url('bankpayment/show_form')?>/"+des+"/"+rem+"/?index="+index,
						//href:"<?=site_url('apinvoice/show_form')?>/"+"/?index="+index,
						onLoad:function(){
...
<form id="formAdd" method="post" action="<?=site_url('bankpayment/saveheader/'.$data->id_cash.'')?>">
<body>
	...
	<tr>	
			<td>No Cheque</td>			
			<td>:</td>
			<td><input type="text" name="cheque" id="cheque" class="validate[required] xinput" xinput" value="<?=@$data->slipno?>" size="30" /></td>
			<td>Amount</td>			
			<td>:</td>
			<td><input type="text" class="calculate input validate[required]" name="amount" id="amount"  value="<?=number_format(@$data->amount)?>"  size="30" /></td>
	</tr>
	<tr>
	<td>Penerima</td>
			<td>:</td>
			<td><input type="text" name="terima" id="terima" class="validate[required] xinput" xinput" value="<?=@$data->from?>"  size="30" /></td>
			
		<td>Remark</td>
			<td>:</td>
			<td><input type="text" name="remark" id="remark" class="validate[required] xinput" xinput" value="<?=@$data->descs?>"   size="30" /></td>
...
<div class="easyui-tabs" style="width:1000px;height:300px;">
	<div title="Add Jurnal Bank" style="padding:10px;">	
		<table id="dg" title="Jurnal Bank" style="width:980px;height:250px"
				url="<?=site_url('bankpayment/get_dg')?>/<?=$data->voucher?>"
