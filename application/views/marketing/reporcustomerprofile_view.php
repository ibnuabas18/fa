...
function loadData(type,parentId){
      ...
      $.post('<?=site_url('reporcustomerprofile/loaddata')?>', //request ke fungsi load data di inputAP
...
$(function(){ 
	...
	$('#subproject').change( 
		...
		function(){
			if($('#ket option:selected').val() != '')
			$('#tipe').empty();
				loadData('kat',$('#subproject option:selected').val());				
		}
	);
		$('#kat').change( 
		function(){
			...
			//~ if($('#tipe option:selected').val() != '')
				...
				$.getJSON('<?=site_url('reporcustomerprofile/gettype')?>/'+ a,
							function(data){
								$('#tipe').empty(); 
...
<h2><font color='red' size='4'>Report Customer Profile<hr width="150px" align="left"></font></h2>
<div class="printed">
<form method="post" action="<?=site_url('reporcustomerprofile/print_customerprofile')?>" target="_blank">
...
-->
	<tr><td colspan='2'><input type="submit" name="save" value="Print" style="width:100px"/></td>
<!--
	<td ><input type="submit" name="excel" value="Print to Excel" style="width:100px"/></td>
-->
	</tr>
	</table>
