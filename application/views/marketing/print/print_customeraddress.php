<?php
			...
			if ($pt==44){			
			$sql = 	"SELECT customer_id,unit_no,customer_nama,customer_alamat2,customer_hp,nama, a.id_subproject, nm_subproject FROM db_sp a
					JOIN db_customer b ON a.id_customer = b.customer_id
					JOIN db_unit_yogya ON id_unit = unit_id
					JOIN db_kary ON a.id_sales=id_kary
					JOIN db_subproject d on d.subproject_id=a.id_subproject
					WHERE ISNULL(a.id_flag,0) <> 10 and unit_id=$unit
					ORDER BY customer_nama";
			$rw = $this->db->query($sql)->result();
			}elseif ($pt==11){
			$sql = 	"SELECT customer_id,unit_no,customer_nama,customer_alamat2,customer_hp,nama, a.id_subproject, nm_subproject FROM db_sp a
					JOIN db_customer b ON a.id_customer = b.customer_id
					JOIN db_unit_bdm c ON id_unit = unit_id and a.id_subproject=c.id_subproject
					JOIN db_kary d ON a.id_sales=id_kary
					JOIN db_subproject e on e.subproject_id=a.id_subproject
					WHERE ISNULL(a.id_flag,0) <> 10 and unit_id=$unit
					ORDER BY customer_nama";
