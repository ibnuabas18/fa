...
class print_kartu_piutang extends controller{
	function kartu_piutang($id){
				...
				//var_dump($id);
				$query = $this->db->query("ViewCustomer_new " .$id."");
			$data	= $query->result();	
...
//					$pdf->SetXY(30,82);
			...
			$lunas = $this->db->query("ViewCustomer_new " .$id."");
			$pl	= $lunas->result();	
	...
	if ($pt==11 || $pt==22 || $pt==66){
	$sumbilling = $this->db->select('sum(kwtbill_pay) as pay_amount')
							->where('id_bill',$row->bank_no) 
							->where_not_in('id_flag',10)
							->get('db_kwtbill')->row();
	}else{						
	$sumbilling = $this->db->select('sum(kwtbill_pay) as pay_amount')
							->where('id_bill',$row->id_billing) 
							->where_not_in('id_flag',10)
							->get('db_kwtbill')->row();
	}
...
//				$amount = number_format($amount);
				...
				$os = $row->amount - $sumbilling->pay_amount;
			...
			// }
				...
				if ($row->fo_amount > 0){
				$pdf->Cell(30,6,$row->paygroup_nm  ." ".$row->id_payjns,1,0,'L',0);
				}else{
