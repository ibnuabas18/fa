<?php
class print_detail_customerservice extends controller{
	function index(){
			...
			$data1 = $this->db->query("PrintCSDetail '" .$customer."','".$start_date1."','" .$end_date1."','" .$checkbox."'");
				$data = $data1->result();
				#var_dump($data);
			...
			$judul 		= "Customer Complaint Detail Report";
			$periode	= "Periode";
			$mgr		= "Sales Name";
			...
			$pdf->Cell(50,6,'Complaint Type',1,0,'C',1);
			$pdf->Cell(60,6,'Complaint Desc',1,0,'C',1);
			$pdf->Cell(40,6,'Project',1,0,'C',1);
			...
			foreach($data as $row){
				...
				if($no == $max){
						...
						#END
								...
								$pdf->Cell(41,8,$row->customer_nama,20,0,'L');
								$pdf->Cell(26,8,$row->customer_hp,20,0,'L');
								$pdf->Cell(51,8,$row->cstipe_nm,20,0,'L');
								$pdf->Cell(61,8,$row->csdesc_nm,20,0,'L');
								$pdf->Cell(41,8,$row->nm_subproject,20,0,'L');
								$pdf->Cell(26,8,$row->csfustat_nm,20,0,'L');
