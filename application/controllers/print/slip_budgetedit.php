<?
Class slip_budgetedit Extends controller{
	...
	function index(){		
		...
		#cek form kode
		$cek_formkode = $this->db->select('form_kode as id')
							   ->where('id_trbgt',$noid)
							   ->get('db_trbgtdiv')->row();
			$formkode = array
			(
			...
			// $this->db->where('kode_form',$cek_formkode->id)
                          // ->update("history_form_print",$formkode);		
			$this->db->where('id_trbgt',$noid)
                          ->update("db_trbgtdiv",$formkode);
