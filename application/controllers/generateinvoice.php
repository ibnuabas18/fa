<?php
class generateinvoice extends DBController{
	function __construct(){
		parent::__construct('generateinvoice_model');
		$this->set_page_title('Generate Invoice');
		$this->default_limit = 30;
		$this->template_dir = 'leasing/generateinvoice';
	...
	function index(){
		$this->set_grid_column('kd_tenant','ID',array('hidden'=>true,'key'=>true));
		...
		$this->set_jqgrid_options_ceklist(array('width'=>1200,'height'=>400,'caption'=>'Generate Invoice','rownumbers'=>true,'sortname'=>'nm_subproject','sortorder'=>'ASC'));
		parent::index();
	}
	...
	function approve($id){     
			...
			extract(PopulateForm());
			 ...
			 $q=$this->db->query("update db_invoice set tgl_invoice='".inggris_date($billdate)."', description='".$descs."' WHERE kd_tenant='".$kd_tenant."' and tgl_invoice='".inggris_date($tgl)."'");
	...
	function view_invoice(){
		  //die("tes");
			...
			// die("$tgl");
				...
				$w=$this->db->query("update db_invoice set id_flag=0, generate_date=NULL WHERE tgl_invoice='".inggris_date($tgl)."' and id_subproject<>'".$proj."' and id_flag=5");
				$s=$this->db->query("update db_invoice set id_flag=0, generate_date=NULL WHERE tgl_invoice<>'".inggris_date($tgl)."' and id_subproject<>'".$proj."' and id_flag=5");
				$s=$this->db->query("update db_invoice set id_flag=0, generate_date=NULL WHERE tgl_invoice<>'".inggris_date($tgl)."' and id_subproject='".$proj."' and id_flag=5");
				$q=$this->db->query("update db_invoice set id_flag=5, generate_date='".inggris_date($tgl)."'  WHERE tgl_invoice='".inggris_date($tgl)."' and id_subproject='".$proj."' and id_flag=0");
           
                  
                  echo"
					<script type='text/javascript'>
						
						document.location.href = 'generateinvoice'
					</script>
				  ";
             //   }
             //  } 
