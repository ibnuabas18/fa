<?
	...
	class alokasi_status extends AdminPage{
		...
		//	vardump($data['view']);exit;						 
			...
			$this->loadTemplate('sales/alokasi_status_view');
		}
		...
		function view_alokasi(){
			...
			$this->parameters['data'] = $data;
			if($proj=='41011'){
				$this->loadTemplate('sales/alokasi_unit_townhouse_view');
			}elseif($proj=='41012'){
				$this->loadTemplate('sales/alokasi_unit_condotel_view');
			}else{
		...
		function form_alokasi($id){
			...
			$this->load->view('sales/reserved_condotel_unit_view',$data);
		}	
		...
		//	vardump($data['cek']);exit();								
			$this->load->view('sales/reserved_town_unit_view',$data);
