<?
	...
	class unit_status extends AdminPage{
		...
		function index(){	
			...
			$this->loadTemplate('marketing/unit_status');
		}
		...
		function view_unit(){
			...
			if($proj=='41011'){
				$this->loadTemplate('marketing/view_unit_townhouse');
			}elseif($proj=='41012'){
				$this->loadTemplate('marketing/view_unit_condotel');
			}else{
		...
		function alokasi_unit(){
			...
			$this->parameters['data'] = $data;		
			$this->loadTemplate('marketing/alokasi_unit');	
		}
		...
		function form_unit($id){
			...
			$this->load->view('marketing/detail_condotel_unit',$data);
		}		
		...
		function form_alokasi($id){
			...
			$this->load->view('marketing/detailalokasi',$data);
		}
		...
		function form_alokasi_town($id){
			$data['town']= $this->db->join('db_unitstatus','unitstatus_id = status_unit')
									->where('unit_id',$id)
									->get('db_unit_yogya')->row(); 						
			$this->load->view('marketing/detail_town_alokasi',$data);
		}		
