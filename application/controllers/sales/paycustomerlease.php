...
class paycustomerlease extends AdminPage{


	function paycustomerlease()
	{
		parent::AdminPage();
	...
	function index(){		
		...
		$this->loadTemplate('sales/paycustomerlease_view');		
	}
	...
	function tampilbill(){
		...
		#TAMPIL NAMA PROYEK
		$sql = "SELECT * FROM db_subproject where subproject_id = $subproject and pt_id=12";
		...
		$sql = "SELECT * FROM db_loo_sewa WHERE id = $unit";
		$roow = $this->db->query($sql)->row();
		$data['roow'] =$roow;
		...
		$data['cek'] = $this->db->query("tampilbill_leasing ".$customerid.",".$unit.",".$pt."")->result();				
		$data['cekbill'] = 'cekbill'; 				 
		$this->parameters['data'] = $data;
		
		$this->loadTemplate('sales/paycustomerlease_view',$data);		
	...
	function loaddata(){
		...
		function cekbilling($id,$proj){
			...
			$rows = $this->db->join('db_customer b','b.customer_id = a.id_customer')
							// ->where('a.status','3')
							 ->where('a.id',$id)
							// ->where('b.status',2)
							 ->where('a.id_subproject',$proj)
							 ->get('db_loo_sewa a')->row();
	  ...
	  # var_dump($xpayun);exit;
	   if($tipe1==6){
		   ...
		   $coa = $rows->bank_coa;
			...
			,".$tipe.",'".@$bank."','".@$remark."','".inggris_date($tgl)."','".$coa."',".replace_numeric($bill[$i]).",'".$remark."',".$komisi.",'".$nm."','".$coa."',".$charge."");	   
			die("sukses");		   
	   
	   // }else if($bank4==32 && $tipe1==2){
			// //$jumunpay = $pay + $xpayun;
			// $sql = $this->db->query("SP_customer_payuntransfer ".$cek[$i].",'".$bill."','".inggris_date($tgl1)."',".replace_numeric($bill[$i]).",".$blc."
			...
			,'".@$bank."','".inggris_date($tgl1)."',".replace_numeric($bill[$i]).",'".$remark."',".replace_numeric($bill[$i]).",".$charge."");	   
			die("sukses");
		   //die("unCredit Card");
	   }elseif($tipe1==3){
			...
			,".$tipe1.",'".@$bank."','".@$remark."','".inggris_date($tgl1)."','".$bank_coa."',".replace_numeric($bill[$i]).",'".$remark."',".$komisi.",".$charge."");	   
			}
			die("sukses");	   
		   //die("credit card");
	   }else{
			...
			if  ($bill[$i]>0){
			$sql = $this->db->query("SP_PaymentCustomer ".$cek[$i].",".$adm.",'".@$remark."','".$no."','".inggris_date($tgl1)."',".replace_numeric($bill[$i]).",".$blc."
			,".$tipe1.",'".@$bank4."','".@$remark."','".inggris_date($tgl1)."','".$bank4."',".replace_numeric($bill[$i]).",'".$remark."',".$komisi."");	  
			}else{
			die("sukses");
			}
			}
		...
		function bayar(){
         ...
         // var_dump($ja);
		   ...
		   $query = $this->db->query("sp_Insertpaymentlease '".$unit1."','".$bank1."','".$remark."','".$subproject1."',".replace_numeric($balance).",".replace_numeric($amount1).",'".inggris_date($tgl1)."',".replace_numeric($bill[$i]).",".$cek[$i].",".$charge.",'".$tipe1."',".$cekkomisi.",".$unid1."");   
            }
			}else {
			for($i=0; $i<$ja;$i++){
         // var_dump($ja);
		...
		#TAMPIL NO UNIT
		//if($pt == 22){
		$sql = "SELECT * FROM db_loo_sewa WHERE kd_tenant= '".$unit1."'";
		$roow = $this->db->query($sql)->row();
		$data['roow'] =$roow;
		...
		$id_unit= $roow->id;
		$customer =$roow->id_customer;
		
		
		
		
		
		$data['cek'] = $this->db->query("tampilbill_leasing ".$customer.",".$id_unit.",".$pt."")->result();					
		$data['cekbill'] = 'cekbill'; 				 
		$this->parameters['data'] = $data;
