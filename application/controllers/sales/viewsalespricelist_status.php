<?
	...
	class viewsalepricelist_status extends AdminPage{
		function viewsalespricelist()
		{
			parent::AdminPage();
		...
		function view_salespricelist(){
			...
			if($view=="ALL"){
				...
				if($pt == 44){
					...
					$data['unit'] = $this->db->where('phase','41013')
										// ->order_by('substring(unit_no,3,4)','ASC')
										 ->order_by('no','ASC')
										 ->get('db_pricelist')->result();
			...
			$this->parameters['data'] = $data;
				...
				$this->loadTemplate('sales/salespricelistsardjito_view');
				}
				elseif($pt==11){
