<?
	...
	class sales extends AdminPage{

		function alokasi_status()
		{
			parent::AdminPage();
		...
		//	vardump($data['view']);exit;						 
			...
			$this->loadTemplate('sales/sales_status_view');
		}
		...
		function view_sales(){
			...
			$this->parameters['data'] = $data;
			if($proj=='41011'){
				$this->loadTemplate('sales/sales_unit_townhouse_view');
			}elseif($proj=='41012'){
				$this->loadTemplate('sales/sales_unit_condotel_view');
			}else{
				die("Belum Ada Gambar");
			}			
		}
		
		function form_sales($id){
			$data['cek']= $this->db->join('db_unitstatus','unitstatus_id = status_unit')
									->where('unit_id',$id)
									->get('db_unit_yogya')->row();	
											
			$this->load->view('sales/sales_condotel_unit_view',$data);
		}	
		
		function form_sales_town($id){
		...
		//	vardump($data['cek']);exit();				
			...
			$this->load->view('sales/sales_town_unit_view',$data);
