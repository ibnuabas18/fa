<?
	...
	class viewunitocea_status extends AdminPage{

		function viewocea()
		{
			parent::AdminPage();
			$this->pageCaption = 'Unit Status Ocea Condotel';
		}
		...
		function index(){	
			...
			$data['view'] = $this->db->select('DISTINCT(view_unit)')
									...
									->get('db_unit_ocea')->result();							  
			#$data['sold'] = 
		//	vardump($data['view']);exit;						 
			$this->parameters['data'] = $data;
			//$this->loadTemplate('sales/unitoceastatus_view');
			$this->loadTemplate('sales/unitoceastatus_view');
		}
		
		function view_unitocea(){
			//die('tes');
			extract(PopulateForm());	
			...
			$this->parameters['data'] = $data;
			$this->loadTemplate('sales/salesadminunitocea_view');
			/*
			if($proj=='41011'){
		...
		function form_salesadmin_condotel($id){
			...
			*/
			...
			/*$data['cek']= $this->db->join('db_unitstatus','unitstatus_id = status_unit')
									...
									//->join('db_sp','id_unit = db_unit_ocea.unit_id')
			...
			if ($data['cek']->unitstatus_id == 1){
				...
				#die("coba");		
				$this->load->view('sales/view_afterocea',$data);
	...
	*/
		//	vardump($data['cek']);exit();				
			...
			$this->load->view('sales/ocea_view',$data);
			...
			if($this->input->post('data_type')){
				...
				//						->join('db_unit_ocea','unit_no = id_unit')
		...
		function updatestatus(){
			...
			$query = $this->db->query("sp_Updatereservedocea '".$unit."','".$aksi."','".$custnm."','".$sales."','".$amount."','".inggris_date($tgl)."'");
			//die("sukses");
