<?php
	class mastervendorgmi extends DBController{
		...
		function edit_vend($id){
			...
			$sql = "SELECT  
					pemasokmaster.kd_supplier as kd_supplier, db_subproject.nm_subproject as nm_project, db_subproject.subproject_id,
					pemasokmaster.nm_supplier as nm_supplier, pemasokmaster.alamat as alamat, pemasokmaster.kota as kota, 
					pemasokmaster.kdkel_usaha as kdkel_usaha, pemasokmaster.telepon as telepon, pemasokmaster.fax as fax, 
					PemasokMaster.npwp, PemasokMaster.akte, PemasokMaster.kodepos, PemasokMaster.kontak
					FROM pemasokmaster
					JOIN pemasok ON pemasok.kd_supp_gb = pemasokmaster.kd_supp_gb
					JOIN db_subproject ON pemasok.kd_project = db_subproject.subproject_id
					WHERE id_pt = $pt and pemasokmaster.kd_supplier = $id
					GROUP BY 
					pemasokmaster.kd_supplier, pemasokmaster.nm_supplier, db_subproject.nm_subproject, db_subproject.subproject_id,
					pemasokmaster.alamat, pemasokmaster.kota, pemasokmaster.kdkel_usaha, pemasokmaster.telepon, 
					pemasokmaster.fax,PemasokMaster.npwp, PemasokMaster.akte, PemasokMaster.kodepos, PemasokMaster.kontak
					";
			$data['ed'] = $this->db->query($sql)->row();
			//var_dump($data['ed']);exit();
			
			$this->load->view('accounting/mastervendorgmi-form',$data);
		}
		
		function edityes(){
			extract(PopulateForm());
			
			$this->db->query("sp_UpdateVendor ".$kd_supplier1.",'".$nm_supplier1."','".$combobox_project."','".$kelusaha."','".$npwp."','".$akte."',
								'".$kontak."','".$alamat."','".$kota."','".$kodepos."','".$telepon."','".$fax."'");
			die("sukses");
			//redirect('project/mastervendorgmi');
		}
