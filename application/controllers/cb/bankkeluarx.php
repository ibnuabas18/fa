<?php
	class bankkeluarx extends DBController{
		
		function __construct(){
			// deskripsikan model yg dipakai
			parent::__construct('bankkeluarx_model');
			$this->set_page_title('Bank Keluar');
			$this->default_limit = 60;
			$this->template_dir = 'cb/bankkeluarx';
		...
		protected function setup_form($data=false){
			...
			$no = 1;
			$proj = 44;														   
			$sql = $this->db->query("sp_cekbknox ".$no.",".$proj."")->row();
			//var_dump($sql);
		...
		function index(){
			...
			$this->set_grid_column('nm_supplier','Nama Supplier',array('width'=>280, 'formatter' => 'cellColumn'));
			$this->set_grid_column('npwp','NPWP',array('width'=>200, 'formatter' => 'cellColumn'));
		...
		function getdata($id){
			$sql = $this->db->query("SP_ViewCBPOx ".$id."")->row(); 
			die(json_encode($sql));
		}
		...
		function cekdata($id_cash){
			$sql = $this->db->query("SP_Viewbkdetailx ".$id_cash."")->result(); 
		...
		function approve2($id){         
			...
			die("Data Belum Lengkap");
					...
					$query = $this->db->query("sp_approvecbx '".$id."'");
					die('sukses');
					}
	...
	function loadcoa(){			
		...
		function savedetail($nobk){					
					...
					$query = $this->db->query("sp_Insertcashdetailx '".$acc_no."','".$line_desc."',".$amount."");
		...
		function saveheader(){
			...
			//$acc_no = $_REQUEST['acc_no'];
					...
					$query = $this->db->query("sp_InsertBKx '".$trx_type."','".$input_user."','".$voucher."','".$bank."','".inggris_date($tgl)."','".$paid."','".$from."','".$kodecash."','".$apno."','".$no_cek."','".inggris_date($cek_date)."','".inggris_date($paid_date)."',".replace_numeric($amount).",'".$remark."'");
			...
			function Editheader(){
					...
					$query = $this->db->query("sp_EditBKx '".$voucher."','".$nocek."','".inggris_date($cek_date)."','".inggris_date($paid_date)."','".inggris_date($trans_date)."'");
					//$this->load->view('cb/print/print_rpayvoucher',$dtprv);
					die('sukses');				              
			}		
			...
			function Editheader2(){
					...
					$query = $this->db->query("sp_EditBK2x '".$voucher."','".$nocek."','".inggris_date($cek_date)."','".inggris_date($paid_date)."','".inggris_date($paid_date)."'");
					//$this->load->view('cb/print/print_rpayvoucher',$dtprv);
					die('sukses');		              
			}		
