...
class dailycash extends DBController{
	...
	function index(){
		...
		if($this->input->post('submit') == TRUE ) {
			...
			$this->parameters['ptsess'] = $this->db->query("select * from pt wherse id_pt = ".$this->session->userdata('sesspt')."")->row();
			var_dump($this->parameters['ptsess']);
			...
			$this->set_jqgrid_options(array('width'=>1230,'height'=>400,'caption'=>'Daily Cash','rownumbers'=>true,'sortname'=>'id_dailycashdet','sortorder'=>'desc'));
		}
		parent::index();
	}
	...
	function reportdata(){
		if ($this->input->post('cek') == 1) {
		...
		} elseif (($this->input->post('banked') == '') and ($this->input->post('projected') == '')) {
			$data['tanggal'] = inggris_date($this->input->post('date'));
			$data['idpt'] = $this->input->post('pted');
			$data['bankid'] = 0;
			$data['projekid'] = 0;
			...
			$data['idpt'] = $this->input->post('pted');
			$data['bankid'] = 0;
			$data['projekid'] = $this->input->post('projected');
			$this->load->view('cb/print/report_daily_detail',$data);
		} else {
			$data['tanggal'] = inggris_date($this->input->post('date'));
			$data['idpt'] = $this->input->post('pted');
			$data['bankid'] = $this->input->post('banked');
			$data['projekid'] = $this->input->post('projected');
			$this->load->view('cb/print/report_daily_detail',$data);
		}
	}
	...
	{
		$tmp = '';
		$q = $this->db->query("select * from db_subproject where id_pt = ".$id." ")->result();
		//var_dump($q);
		if (count($q) > 0) {
			$tmp .= "<option value=''>Pilih Project</option>";
			foreach ($q as $row) {
				$tmp .= "<option value='" . $row->subproject_id . "'>" . $row->nm_subproject . "</option>";
			}
		} else {
			$tmp .= "<option value=''>Pilih Project</option>";
		}
	...
	{	
		$tmp = '';
		$q = $this->db->query("select * from bank WHERE kd_project = ".$id." and namabank NOT LIKE '%transit%' ")->result();
		//var_dump($q);exit();
		if (count($q) > 0) {
