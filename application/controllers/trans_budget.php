...
class trans_budget extends DBController{
	...
	protected function setup_form($data=false){
		...
		//$this->parameters['a'] = $this->mstmodel->getbudget($divisi,$pt);
		 $this->parameters['bank'] = $this->db->select("code id,' {' +code+ '}' + descbgt  nama")
	...
	function index(){
		...
		function bgt_balance($bln,$thn,$kode){
			...
			$cekdiv = $this->db->where('code',$kode)
							   ->where('thn',$thn)
							   ->where('id_pt',$pt)
							   ->get('db_mstbgt_update')->row();			
			$divisi = $cekdiv->divisi_id;
			//if($level_id == 1) $divisi = $divisi;
			//else $divisi = $divisi_id;
			#get budget
			$datv = $this->mstmodel->getmstbudgetupdate($kode,$thn,$pt);
			...
			$jbln = $arr[$id];
			$tot3 = $this->mstmodel->total_month($kode,$jbln,$thn,$pt);
			$tot4 = $this->mstmodel->act_month($kode,$month,$thn,$pt);
			...
			for($i=1;$i<=$bln;$i++){
				$n = $arr[$i];
				$tot5 = $this->mstmodel->total_ytd($n,$kode,$thn,$pt);
				$hsl = $tot5['jml'];
				$totytd = $totytd + $hsl;
			}			
			$tot6 = $this->mstmodel->act_ytd($kode,$thn,$pt);
			$totytd = $totytd;
			$actytd = $tot6['jml'];
			...
			for($i=1;$i<=$bln;$i++){
				$n = $arr[$i];
				$tot5div = $this->mstmodel->total_ytddiv($n,$divisi,$thn,$pt);
				$hsldiv = $tot5div['jml'];
		...
		function save_edit(){
			...
			(
				'code_id' => $codeid,
				'amount' => str_replace(',',"",$amountedit),		
				'remark' => $remarkedit,		
			);
						
				$this->db->where('id_trbgt',$noid)
                         ->update("db_trbgtdiv",$data);
