<?php
	class bankpayment extends DBController{
		
		function __construct(){
			// deskripsikan model yg dipakai
			parent::__construct('bankpayment_model');
			$this->set_page_title('List Of Employee');
			$this->default_limit = 30;
			$this->template_dir = 'cb/bankpayment';
		...
		protected function setup_form($data=false){
			...
			//var_dump($sql);
			$this->parameters['nocb'] = $sql->no_bk;	
		...
		function index(){
			...
			$this->set_jqgrid_options(array('width'=>1000,'height'=>400,'caption'=>'BANK OUT','rownumbers'=>true));
			parent::index();		
		}		
		...
		function saveheader(){
			...
			// if(@$balance->balance <> $amount_cek){
				// die("Jurnal Tidak Balance");
					...
					$query = $this->db->query("sp_InsertcashheaderNonAp '".$trx_type."','".$voucher."','".$project_detail."','".$bank."','".$cheque."','".$terima."','".inggris_date($tgl)."','".$paid."','".$from."','".$kodecash."',".replace_numeric($amount).",'".$remark."'");
//$this->load->view('cb/print/print_recvoucher',$dtprv);
		...
		function print_slip($id){		
			...
			$this->load->view('cb/print/print_paymentvoucherall',$dtprv);
		}
...
//die($id);					
			...
			function approve_nonap($id){         
			
					$query = $this->db->query("sp_approvecbnonap '".$id."'");
					die('sukses');
...
function loaddata(){
		...
		// $cek_closing = $this->db->select('left(convert(varchar(10), trans_date, 112),6)')
							   // ->where('id_cash',$id)
							   // ->get('db_cashheader')->row();
