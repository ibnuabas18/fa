<?php
class reschedule extends DBController{
	function __construct(){
		parent::__construct('reschedule_model');
		$this->set_page_title('Unit Reschedule');
		$this->template_dir = 'finance/reschedule';
	...
	//die("tes");
		...
		$this->set_jqgrid_options(array('width'=>870,'height'=>300,'caption'=>'All unit with outstanding payment','sortname'=>'customer_nama','sortorder'=>'asc'));
		parent::index();		
	}
	
	function reschedule_view($tgl1,$tgl2,$jum,$val,$id_sp){
		$data['start'] = inggris_date($tgl1);
		$data['end']   = inggris_date($tgl2);
		$data['jml']   = $jum;
		$data['val']   = $val;
		$data['val_dp']= $this->db->query("select * from db_billing where id_sp='".$id_sp."' and id_paygroup=2")->row()->pay_sisa;
			$this->load->view('finance/billingreschedule_view',$data);
		}
	
	function reschedules($id){ 
		$pt = $this->pt_id;
		if($pt == 44){
			$unit = "db_unit_yogya";
		}
