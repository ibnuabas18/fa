...
// disc,discnilai
	...
	$rows = $this->db->query("sp_printpo")->row();
...
// where a.brgpoh_id = 3
	...
	$dt = $this->db->select('nm_supp,alamat,kota,telepon,fax,kontak,no_po,tgl_po,
				   ...
				   ->join('pemasokmaster d','d.kd_supplier = a.kd_supp')
				   ->where('a.brgpoh_id','3')
	...
	$tot = $tot + $row->harga_tot;	
						...
						$pdf->Cell(25,5,number_format($row->harga_sat),1,0,'C');
						$pdf->Cell(10,5,number_format($row->discnilai),1,0,'C');
						$pdf->Cell(25,5,number_format($row->harga_tot),1,0,'C');
						$pdf->Ln(5);
						$i++;
						$no++;
				}
		...
		$pdf->Cell(25,4,number_format($tot),'L'.'T'.'R',0,'C');
		$pdf->Ln(4);
		
		$pdf->SetX(5);
		$pdf->Cell(140,4,'','L'.'R',0,'R');
		$pdf->Cell(35,4,'Discount','L'.'R',0,'R');
		$pdf->Cell(25,4,number_format($row->discvnilai),'L'.'R',0,'C');
		$pdf->Ln(4);
		
		$totsedisc = $tot - $row->discvnilai;
		
		$pdf->SetX(5);
		$pdf->Cell(140,4,'','L'.'R',0,'R');
		$pdf->Cell(35,4,'Total','L'.'T'.'R',0,'R');
		$pdf->Cell(25,4,number_format($totsedisc),1,0,'C');
		$pdf->Ln(4);
		
		$pdf->SetX(5);
		$pdf->Cell(140,4,'','L'.'R',0,'R');
		$pdf->Cell(35,4,'PPN','L'.'T'.'R',0,'R');
		$pdf->Cell(25,4,number_format($row->ppn_nilai),1,0,'C');
		$pdf->Ln(4);
		
		$grandtot = $totsedisc + $row->ppn_nilai;
		
		$pdf->SetX(5);
		$pdf->Cell(140,4,'','L'.'B'.'R',0,'R');
		$pdf->Cell(35,4,'Grand Total','L'.'B'.'R',0,'R');
		$pdf->Cell(25,4,number_format($grandtot),1,0,'C');
		$pdf->Ln(4);
	
		$bilangan 	= UcFirst(toRupiah($grandtot));
	
		$pdf->Ln(1);
		$pdf->SetX(5);
		$pdf->Cell(15,6,'Terbilang : ',20,0,'L');
		$pdf->Cell(185,6,'#'.$bilangan.'#',1,0,'L');
		$pdf->Ln(6);
