<?php
class print_sales_customerprospek extends controller{
	function index(){
			...
			$data1 = $this->db->query("PrintSalesDetail '" .$salesmanager."','".$start_date1."','" .$end_date1."'");
				$data = $data1->result();
				#var_dump($data);
			...
			$judul 		= "Customer Prospective Report";
			$periode	= "Periode";
			$mgr		= "Sales Name";
			...
			$pdf->Cell(23,6,'FU Date',1,0,'C',1);
			$pdf->Cell(40,6,'Prospect Name',1,0,'C',1);
			$pdf->Cell(25,6,'Status',1,0,'C',1);
			$pdf->Cell(30,6,'Mobile Phone',1,0,'C',1);
			$pdf->Cell(40,6,'Sales',1,0,'C',1);
			$pdf->Cell(60,6,'Project',1,0,'C',1);
			...
			foreach($data as $row){
				...
				$pdf->Cell(23,8,$fu_tgl,20,0,'C');
				$pdf->Cell(40,8,$row->customer_nama,20,0,'L');
				$pdf->Cell(28,8,$row->prospectstat_nm,20,0,'C');
				$pdf->Cell(30,8,$row->customer_hp,20,0,'L');
				$pdf->Cell(40,8,$row->nama,20,0,'L');
				$pdf->SetFont('Arial','',7);
