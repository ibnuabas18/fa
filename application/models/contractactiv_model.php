<?php
class contractactiv_model extends DBModel{
	
	function __construct(){ 
		parent::__construct('db_activation_sewa','id');
		$this->set_join('db_customer','db_customer.customer_id = db_activation_sewa.id_customer','left');
		#jhh$this->set_join('db_custemrg','db_customer.customer_id = db_custemrg.id_customer');
	}
	
	function before_fetch(){
		
		$this->db->select('id,no_activation,tgl_activation,tgl_endactivation,tot_sewa, kegiatan,customer_nama');
			//->where('pt_id',12);
	...
	//~ protected function before_fetch(){
			//~ $this->db->select('customer_nama,customer_alamat1');
			//~ $this->db->where('category','rental');
		//~ 
			//~ parent::before_fetch();
		//~ }
