...
class generatefpajak_model extends DBModel{
	...
	function __construct(){ 
		...
		$this->set_join('pemasok','vendor_acct = kd_supplier','left');
	...
	function before_fetch(){
		...
		*/
		...
		#Sementara di Hilangkan buat tes print faktur pajak nya
		$this->db->where('db_apinvoice.status', 3);
		$this->db->where('project_no', $idProject);
		 $this->db->where("inv_date BETWEEN '$s' AND '$e'", NULL, FALSE); 
		parent::before_fetch();
