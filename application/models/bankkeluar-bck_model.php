<?php
	class bankkeluar_model extends DBModel{
		...
		function __construct(){ 
			...
			$pt = $session['id_pt'];
			$status = array('2','3'); 
			$this->db->select('e.trans_date,d.bank_nm,e.id_cash,a.id_plan as id_plan,b.descs,b.doc_no,c.nm_supplier,e.voucher,e.no_arsip,e.amount,e.amt_balance,e.slipno,e.slip_date,e.payment_date,e.status');
	...
	/*class bankkeluar_model extends DBModel{
		...
		function __construct(){ 
			parent::__construct('db_cashplan a','a.id_cash');	
			$this->set_join('db_cashheader e','e.id_cash = a.id_cash','left');
			$this->set_join('db_apinvoice b','b.apinvoice_id = a.id_ap','left');
			$this->set_join('db_apledger c','c.ref_no = b.apinvoice_id', 'left');
			$this->set_join('db_bank d','d.bank_id = e.bankacc','left');
			...
			$this->set_join('db_subproject g','g.subproject_id = b.project_no');
			//$this->set_join('db_cashplan e','e.id_ap = b.apinvoice_id','left');
			...
			$status = array('2','3');
			$this->db->select("d.bank_nm,a.id_plan,a.id_cash,isnull(voucher,'-') as voucher, isnull(convert(varchar(12), trans_date, 105),'-') as trans_date, 
e.status, slipno, isnull(convert(varchar(12), slip_date, 105),'-') as slip_date, isnull(convert(varchar(12), payment_date, 105),'-') as payment_date, 
